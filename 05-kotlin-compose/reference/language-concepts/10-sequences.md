...
listOf(1, 2, 3, 4, 5)
    ...
    .first { it > 4 }                     // 结果 6
// 实测 eagerLog == [1, 2, 3, 4, 5]——map 全跑了一遍
...
listOf(1, 2).map { /* 标记 m$it */ }.filter { /* 标记 f$it */ }
```
...
### 4. 性能对比（本机实测：JVM、200 万元素、filter→map→take(5)）
...
本机实测（连跑 4 轮取稳定值）：**急切链 ~350ms，Sequence 链始终 <1ms**（首轮冷 JVM 含 JIT 预热更高）。差距来源：急切版必须构建并遍历两个百万元素中间集合，Sequence 只让 9 个元素流过整条链。结论：**集合越大、链越长、越早短路，Sequence 收益越大**；几十条数据的 UI 状态变换差距可忽略。

## ⚠️ 常见陷阱

- ❌ 把"Sequence 惰性"理解成"能随便存起来反复用"——**单次迭代限制确实存在，但只作用于一次性来源**：`iterator().asSequence()`（实测第二次 `toList()` 抛 `IllegalStateException: This sequence can be consumed only once.`）与 `.constrainOnce()` 标记的序列。
  ✅ 需要多次消费先 `toList()` 固化；接收 Sequence 参数且来源不可控时，消费前先 `toList()` 最稳。
- ❌ 反向误解：以为 `listOf(...).asSequence()` 也只能迭代一次——**实测可重复迭代**（集合 backed、`generateSequence`、`sequence {}` 构建器每次迭代都重建迭代器，实测 `sequence{}` 第二次 `toList()` 重新产出 `[1, 2]`）。
  ✅ 但"可重复"= 每次迭代**从头重放**：实测同一序列迭代两次，`map` 里的 lambda 执行次数 ×2。
- ❌ 在 Sequence 的 `map`/`onEach`/构建器里写副作用（发请求、写状态）——惰性意味着**不迭代就一行不跑、迭代几次就跑几次**（实测 `sequence { sideEffect(); yield(1) }` 未迭代时构建体不执行）。
...
- ❌ 小集合（几十条）也套 `asSequence()`——每次迭代要创建迭代器对象，收益为负。
