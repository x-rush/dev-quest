...
开工前先在 Android Studio 的空 Compose 工程显示模板页，记录 Android Studio、JDK、Gradle Wrapper、Kotlin/AGP 与设备系统版本。保留模板版本组合；如果连模板都无法构建，先回环境篇解决依赖下载、SDK 或 JDK 问题，不把业务代码混入排查。

| 关卡 | 最小产物与具体输入 | 通过后下一步；未通过回查 |
|---|---|---|
| Kotlin 能力 | 一个纯函数：输入 `listOf(null, "", " A ", "B")`，去掉 null/空白并 trim，返回 `listOf("A", "B")`；附空集合用例 | 能解释每一步输入输出后进入状态篇；依赖 `!!` 才通过则回可空性与集合 |
| UI 数据流 | 一个计数页面，由父组件拥有值，两个子组件通过回调增加和重置；提交源码与 0→1→2→0 操作记录 | 两处值始终一致后进入生命周期；不刷新先查是否使用可观察状态，不用强制重绘掩盖 |
| 首项目持久化 | 可构建的 QuickNotes 工程、版本目录和 Room schema；新增 A/B、删除 B、终止进程后重新打开，记录仅 A；空白标题不得写入 | 按[首项目](basics/08-first-project.md)完成后做[编辑与搜索](projects/01-notes-app.md)；重启丢数据查是否真的使用磁盘数据库，构建成功不能代替设备动作 |
| 编辑与搜索 | 原 id 的 A 改为 A2，取消编辑不改数据；快速切换查询、注入读取/保存失败，提交结果表 | 错误可恢复、无额外插入且搜索不显示错配结果后做天气项目；先回查查询取消、id 传递和错误分支 |

每份记录至少写明“用例名、环境、操作、期望、实际、结论”；没有设备、没有执行的故障路径写“未验证”。旋转是 Activity 配置重建，终止进程后冷启动检验持久化，两者不能互相代替。ViewModel 只在内存中保留状态，简单 UI 状态按需要使用 `rememberSaveable` 或 `SavedStateHandle`，长期笔记写入 Room。官方边界见 [Compose 状态保存](https://developer.android.com/develop/ui/compose/state-saving)。
