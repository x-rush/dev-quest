...
1. **工具按兼容性与证据选择**：先记录目标框架版本、插件约束、生产构建与调试需求，再选择默认打包器或兼容方案；“更快”必须指明冷启动、热更新还是生产构建。
2. **分割改变请求形状**：路由或动态组件分割后检查目标页面的初始资源、额外请求瀑布与交互等待。更小首包可能换来更多网络往返，不能只比较一个文件大小。
3. **Tree shaking 依赖模块边界**：检查生产产物而非源码 import 数量；副作用导入、CommonJS 包装和错误的 barrel 导出都可能保留不需要的代码。
4. **缓存必须能回退与失效**：静态资源使用内容标识，HTML、数据与用户授权数据按各自有效期处理；发布后确认旧客户端不会请求不存在的 chunk。
5. **预算由旅程驱动**：为关键路由设定可审查的资源和交互预算，持续记录 build、设备与网络条件。预算报警是复查入口，不是自动证明用户体验良好。

### 学习成果验收

- [ ] 保存一个关键路由优化前后的生产构建分析，列出减少或新增的资源及其加载时机。
- [ ] 在固定网络条件下验证代码分割前后首屏与交互的等待，说明取舍而非只报告首包大小。
- [ ] 故意引入副作用导入或大依赖，确认产物报告能发现回归；修复后记录差异。
- [ ] 在一次测试发布后访问旧页面或模拟旧 HTML，确认内容哈希资源和回退策略不会造成 chunk 加载失败。
- [ ] 把一个预算阈值关联到具体路由和测量环境，发生超标后能追溯到依赖或代码改动。
