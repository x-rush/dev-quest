...
> **文档简介**: TypeScript核心类型和实用类型快速参考，涵盖基础类型、高级类型、工具类型等
>
...
1. **类型是约束，不是数据校验**：TypeScript 只在编译时检查。URL、表单、`localStorage` 和 API 返回值都可能绕过类型；先把它们当作 `unknown`，再用运行时校验缩小到可信形状。
2. **组件 Props 要表达允许的组合**：用联合类型区分互斥变体，用可选属性只表达真正可缺省的值。不要为了消除报错把 Props 写成 `any`、一串可选字段或不加判断的非空断言。
3. **缩窄优于断言**：通过判别字段、`typeof`、`in`、用户定义类型守卫和显式分支让编译器知道当前值的形状；`as` 只在你已从外部证据确认值正确时使用，不能修复不可信输入。
4. **工具类型服务于领域模型**：`Pick`、`Omit`、`Partial` 和 `Record` 应从稳定的领域类型推导；特别是 `Partial` 不等于更新请求合法，更新 API 仍要检查至少一个字段、字段范围与权限。
5. **泛型保留关联关系**：泛型函数或组件应让输入类型与返回类型保持关联，并设置有意义的约束。若调用方总是只能传一种类型，简单的具体类型往往比抽象泛型更清楚。

### 学习成果验收

- [ ] 把一个 API 响应先标为 `unknown`，为合法、缺字段和错误字段三种输入写运行时校验，再让组件只接收校验后的数据。
- [ ] 为一个按钮或输入组件设计两个互斥变体；故意传入冲突 Props，确认 `tsc --noEmit` 报错。
- [ ] 用判别联合处理加载、成功与失败状态，删除一个分支后让穷尽性检查或测试暴露遗漏。
- [ ] 为一个更新 DTO 选择恰当的工具类型，并证明空对象、未知字段和越界值仍会在运行时被拒绝。
- [ ] 写一个泛型 `mapById` 或列表组件，证明传入元素类型会推导到回调和返回值；再说明其约束为何必要。
...
### 官方文档
- 📖 **[TypeScript 官方文档](https://www.typescriptlang.org/docs/)**: 完整TypeScript文档和类型系统说明
