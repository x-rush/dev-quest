...
以下签名摘自 `@types/react@19.3` 与 `@types/react-dom@19`：
...
function PostFeed({ postsPromise }: { postsPromise: Promise<Post[]> }) {
...
- 传给 `use()` 的 Promise 必须引用稳定：在渲染体内直接 `use(fetch(...))` 会每帧新建 Promise，导致无限挂起或重复请求。应传入 Server Component 里创建的 Promise，或 `'use cache'` 函数的返回值。
...
- `useActionState` 第三参数 `permalink` 仅用于多标签页/水合前提交的场景（跳转到稳定的permalink 页面再展示结果），常规表单不需要传。
