...
1. **文件约定决定入口，组件边界决定能力**：`page`、`layout`、`loading`、`error`、`not-found` 与 `route` 各解决不同入口和状态。默认服务端组件能读取服务端资源；一旦需要事件处理、浏览器 API 或本地状态，再把最小交互子树标为客户端组件。
2. **动态参数仍是不可信输入**：路由段、查询参数和请求正文要先检查格式、范围和资源归属。TypeScript 的 `params` 类型只能帮助调用方，不能阻止用户手写 URL 或绕过界面访问别人的资源。
3. **读取、缓存、写入要连成闭环**：为每次读取说明缓存键、失效时间或标签；写入成功后刷新或失效受影响的数据。对用户特定、频繁变化或需要强一致的内容，明确选择不缓存或受控重新验证的原因。
4. **中间件适合早期、轻量决策**：重定向、区域或粗粒度访问控制可以放在中间件；最终资源授权必须在服务端动作、路由处理器或数据访问层再次执行，不能把中间件当成唯一安全边界。
5. **配置按公开性分层**：只有设计为公开的变量才可暴露给浏览器。构建时和运行时的变量来源、缺失时行为以及密钥轮换方式应写入部署说明，日志中不能打印秘密值。

### 学习成果验收

- [ ] 为一个页面分别实现加载、错误、找不到和成功状态，在浏览器或测试中验证每种状态的可见输出。
- [ ] 手工修改动态路径和查询参数，确认无效格式、越界值、缺失资源和越权资源各返回恰当结果。
- [ ] 写入一条记录后返回列表，确认缓存已经刷新；再记录所用缓存标签或重新验证策略及原因。
- [ ] 绕过中间件直接调用写入端点，确认服务端仍验证当前身份和资源归属。
- [ ] 在干净环境构建或启动一次，分别验证公开变量可用、必需密钥缺失会明确失败、日志不输出密钥内容。
