...
1. **组件是纯渲染描述**：同样的 Props、State 与 Context 应产生同样的 JSX。计算派生值放在渲染阶段；修改网络、订阅、计时器或浏览器 API 才属于 Effect 的外部同步。
2. **Hooks 调用顺序必须稳定**：只能在组件或自定义 Hook 的顶层调用，不能放入条件、循环和普通回调。条件需求应在 Hook 内部处理或拆为两个组件，否则状态槽位会错位。
3. **Effect 必须可清理且抗竞态**：订阅、定时器和请求在依赖变化或卸载时取消或忽略旧结果；依赖数组列出 Effect 读取的响应式值，不能用禁用 lint 规则逃避更新错误。
4. **列表 key 表示身份，不是位置**：使用稳定业务 ID，使插入、删除、排序后组件状态仍跟随同一条数据。数组下标只适合永不重排、永不插入也不含局部状态的静态列表。
5. **memo 化先测量再使用**：`useMemo`、`useCallback` 和 `memo` 只在昂贵计算或由引用变化引发的真实重渲染问题中使用；它们会增加依赖管理成本，不能替代正确状态划分。

### 学习成果验收

- [ ] 把一个渲染阶段的派生 `fullName` 或筛选结果从 Effect 移回渲染，确认不再产生额外一次渲染或短暂旧值。
- [ ] 故意在条件中调用 Hook，观察 lint 或运行时报错后，重构为稳定调用顺序。
- [ ] 为一个订阅或异步请求添加清理，快速切换输入或卸载组件，确认旧结果不会覆盖新界面。
- [ ] 给可排序列表使用业务 ID 作为 key，输入一行局部状态后排序，确认该状态仍属于同一条记录。
- [ ] 使用 React DevTools Profiler 或可重复的测量记录一个真实重渲染问题，再只添加必要的 memo 化并比较前后结果。
