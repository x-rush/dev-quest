...
> **前置知识**: Next.js 16基础、React 19、TypeScript 7、表单基础概念、Zod验证库
...
### 🏗️ 企业级最佳实践
表单先保证用户能理解标签、发现错误并修正：每个字段说明格式，提交失败保留已输入内容，把错误与对应字段关联，并验证键盘与读屏操作。大表单性能问题先测量，再决定分步、延迟渲染或代码拆分。

客户端验证改善反馈，服务端重新检查输入和权限；提交按钮禁用不等于防止重复写入，必要操作需要服务端幂等。分析事件只采集必要信息，避免把填写内容当作普通埋点。用超时、重复点击和跨字段错误测试恢复路径。
...
本章的 schema、Hook 和动态字段模式用于建立可维护的表单边界；它们不自动覆盖每个业务规则、文件安全或服务端授权。上线前以服务端校验为准，并对真实输入、网络失败和无障碍交互进行独立验证。

### 表单交付验收

1. 客户端与服务端针对空值、格式、跨字段规则和未知字段得到一致结果；绕过浏览器直接请求仍会被服务端拒绝。
2. 异步校验在输入改变、页面卸载或较慢旧请求返回时不会覆盖当前字段状态。
3. 动态增删字段后，值、默认值、错误和 schema 规则同时注册或移除；提交内容中没有幽灵字段。
4. 上传先限制大小、类型和数量，服务端按内容与权限复核；文件名和 MIME 声明不能单独作为信任依据。
5. 键盘可以到达每个控件，错误与对应字段关联，提交中和失败后仍保留可修正输入。
...
### 参考章节
- 📖 **[本模块其他章节]**: [客户端组件模式](./03-client-components-patterns.md#3-事件处理模式)中的表单事件处理部分
...
- [ ] 对一个 schema 覆盖正常输入、缺失字段、未知字段、跨字段冲突和服务端业务拒绝；客户端提示与服务端结果不矛盾。
- [ ] 通过键盘完成填写、查看错误、修正、提交和失败后重试；错误消息与字段关联，提交中不丢失用户输入。
- [ ] 快速修改异步校验字段并延迟旧响应，确认当前值不会被旧成功或失败状态覆盖。
- [ ] 动态添加、删除和重排字段后，检查注册状态、默认值、错误和提交 payload 不包含残留字段。
- [ ] 对文件上传验证大小、类型、数量、服务端内容检查和资源归属；客户端 accept 属性不是安全边界。
