...
Next.js 内置 `.env*` 文件加载（写入 `process.env`）与 `NEXT_PUBLIC_` 前缀内联机制。核心规则一句话：**没有 `NEXT_PUBLIC_` 前缀的变量只存在于服务端；带前缀的变量在 `next build` 时被文本替换进客户端 bundle，构建后不再随环境变化**。理解"构建期内联 vs 运行时读取"的边界，是同时保住 secret 安全与多环境部署的关键。
...
- ❌ **动态查找 `NEXT_PUBLIC_` 变量**：`process.env[varName]` / `const env = process.env` 不会被内联，运行时是 `undefined`。✅ 始终用静态点访问 `process.env.NEXT_PUBLIC_X`。
- ❌ **`.env.local` 提交进仓库或以为测试会加载它**：`.env*.local` 按约定进 `.gitignore`（create-next-app 默认已配）；且 `NODE_ENV=test` 时 `.env.local` 被跳过。✅ 团队共享默认值放 `.env` / `.env.test`（可入库），个人覆盖放 `.env.*.local`。
