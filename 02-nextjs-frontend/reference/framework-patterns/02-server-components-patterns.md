...
前置：组件、props、异步函数。服务端组件可以在服务器读取受保护资源，再把可序列化结果交给界面。客户端组件负责事件与交互；`use client` 定义模块边界，不是简单的“只在浏览器渲染”开关。

设页面上有商品说明和“加入收藏”按钮：服务器读取商品说明，客户端按钮接收商品 ID 并处理点击。这个拆分把交互依赖限制在需要它的部分，但提交收藏的服务端入口仍须校验身份和资源权限。

自测：商品说明里能否读取数据库密钥？它应留在服务器。是否可以把带有秘密字段的整个数据库对象作为 props 传给客户端？不能因为它来自服务端组件就假定不会泄露；要明确选择客户端需要的字段。查看 HTML 或网络载荷中的字段才能验证传输了什么。
...
> **前置知识**: Next.js 16基础、React 19组件概念、TypeScript 7、缓存机制、数据库基础、API设计
...
- **服务端组件的默认产物是不进入客户端包的渲染结果**：组件在服务器执行完毕后只下发 UI，交互能力缺失是架构特性而非缺陷，任何交互状态必须显式下沉到客户端边界（对照概述"零 JavaScript 发送到客户端"与全文各模式）。
- **同一次请求内对同一数据的多次获取必须收敛为一次**：请求级去重的作用域跟随请求而非组件，去重包装必须创建在模块顶层——写在函数体内会随每次调用重建而彻底失效（对照"基础数据获取组件"中 `cache()` 的使用与注释）。
- **缓存是显式声明，而非隐式默认**：每份数据按其新鲜度显式声明策略——实时数据不缓存、敏感数据强制 `no-store`、长尾数据声明生命周期与失效标签（对照"安全数据处理"中敏感资源的 `cache: 'no-store'` 与 `"use cache"` 显式缓存）。
- **慢数据只推迟自己的插槽，不阻塞已就绪内容**：等待被 Suspense 边界隔离到最小粒度，先交付首屏可用部分，占位符明确表达尚未就绪的范围（对照"流式渲染模式"的流式列表与实时数据流）。
- **服务端是唯一信任边界**：进入渲染的输入先验证、输出到 DOM 前先清理、操作前先裁决权限，客户端提交的一切视为不可信（对照"安全性模式"的 schema 验证、HTML 清理与权限检查）。
...
### 参考章节
- 📖 **[本模块其他章节]**: [App Router模式](./01-app-router-patterns.md#-根布局与模板系统)中的布局系统部分
