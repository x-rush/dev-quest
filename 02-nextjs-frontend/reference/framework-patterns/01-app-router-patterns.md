...
> **前置知识**: Next.js 16基础、React 19组件概念、TypeScript、文件系统、HTTP协议
...
- [ ] 从一个静态页面、一个动态资源页和一个共享 layout 出发，直接访问、刷新和浏览器后退均有可解释的 URL 与内容。
- [ ] 为动态段和查询参数覆盖缺失、重复、无效、越界、资源不存在与无权限访问；服务端而非类型声明决定最终拒绝。
- [ ] 对每个共享 layout 写出哪些状态跨子路由保持、哪些必须随页面销毁，避免把局部交互错误提升到根布局。
- [ ] 在并行插槽中分别模拟加载和失败，确认一个插槽故障不会覆盖另一个插槽的可用内容。
- [ ] 从列表打开拦截路由后测试刷新、直接复制 URL、关闭覆盖层和进入不存在资源，确认每条路径有稳定退路。
