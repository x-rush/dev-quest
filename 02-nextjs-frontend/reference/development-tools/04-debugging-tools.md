...
> **前置知识**: Next.js 16基础、TypeScript 7、JavaScript调试基础、浏览器开发者工具
...
构建排查先区分源码解析、依赖解析、打包与运行错误，保留实际命令、版本和第一条相关诊断。source map 把产物位置对应到源码，生产是否公开应按调试与信息暴露需求安排。

tree shaking、动态导入和压缩解决不同问题，也不保证所有未调用代码都会自动移除。构建提速用冷缓存与热缓存分别测量，避免缓存掩盖真实依赖。插件与构建脚本会执行代码，凭据和权限需限制；最终产物完整性不能由“构建成功”单独证明。
