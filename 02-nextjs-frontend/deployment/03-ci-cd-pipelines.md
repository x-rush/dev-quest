# Next.js 16 CI/CD 流水线完全指南

> **文档简介**: Next.js 16 + 现代CI/CD流水线完整指南，涵盖GitHub Actions、自动化测试、多环境部署、代码质量检查、安全扫描等企业级CI/CD实践

> **目标读者**: 具备Next.js基础的开发者，需要掌握企业级CI/CD流水线设计和DevOps实践的工程师

> **前置知识**: Next.js 16基础、Git版本控制、Docker基础、CI/CD概念、Linux基础
...
### 🛠️ DevOps最佳实践
把基础设施与发布配置纳入版本控制，能回答“这次发布改变了什么”和“如何还原”。蓝绿或金丝雀只有在流量切换、健康判断与旧版本兼容明确时才有价值，不要求每个练习同时实现两套方案。

流水线先在受控环境演练失败：测试不通过不能发布，新实例未就绪不能接流量，部署后业务冒烟失败能停止放量。权限按检查、构建与发布分开，日志记录足够定位失败但不输出凭据。

## 📖 概述

Next.js 16 应用的CI/CD流水线是现代软件工程的核心组成部分，直接影响开发效率、代码质量和系统稳定性。本指南深入探讨企业级CI/CD实践，从基础的工作流配置到复杂的部署策略，涵盖质量保证、安全扫描、监控告警等全方位内容，帮助你构建世界级的自动化交付体系。
...
验收流水线时故意提交一次测试失败，确认发布被阻止；再用同一通过检查的产物部署，核对版本标识与核心功能。可以并行的检查独立运行，有依赖的构建和发布按顺序衔接，重试只用于可安全重试的步骤。

测试分层按风险覆盖：纯逻辑检查规则，集成检查协作，E2E 检查核心旅程。部署后的回退要区分应用产物、配置和数据库迁移；秘密仅注入可信步骤，审计与许可证检查结果需要处理记录，而不只是一个绿色徽章。

## 🎯 总结

Next.js 16 CI/CD流水线是现代软件工程的核心实践，直接影响开发效率、代码质量和交付速度。通过合理设计工作流、配置自动化测试、实施质量门禁，可以构建可靠的自动化交付体系。
...
### 相关知识模块
- [框架相关模块](../frameworks/01-nextjs-16-complete.md) - Next.js 16的构建和部署集成
...
1. **工作流触发与权限**：列出哪个事件会运行哪些 job，发布 job 使用的最小权限和需要保护的环境。不要让来自外部 PR 的内容获得部署凭据。
2. **门禁顺序**：先运行可重复的安装、静态检查、单元测试和构建；让一次故意失败的检查证明镜像推送与部署不会启动。缓存只能缩短时间，不能跳过验证。
3. **环境与产物**：同一 commit 生成不可变的产物标识（如镜像 digest），测试、预发布和生产部署记录该标识、目标环境和运行号；环境差异放在受控配置与密钥中，不能写进镜像或日志。
4. **部署决策**：滚动、蓝绿或金丝雀发布须明确健康检查、观察窗口、停止条件与回退的已知好版本。策略名称本身不能说明发布安全。
5. **变更可追溯**：一次成功或失败运行都应能反查 commit、执行的命令、产物与部署结果；GitOps 只是把期望状态存进版本库，仍要验证控制器实际同步结果。

### 学习成果验收

- [ ] 提交一个隔离分支的故意失败测试或构建，保存失败 job 日志，并确认发布 job 被跳过或阻断。
- [ ] 修复后保存成功运行的 commit SHA、产物 digest、部署环境和健康检查结果。
- [ ] 用相同不可变产物部署到测试环境，验证发布记录没有依赖 `latest` 这样的可变标签。
- [ ] 演练一次回退：记录触发条件、回退版本、健康检查和一个真实页面或 API 请求的结果。
- [ ] 检查 Actions 日志、构建产物和镜像层不包含令牌、环境变量值或用户数据。
...
- 选择一个已有小项目，在测试环境保留一次失败门禁、一次成功发布和一次回退的运行链接；这三份记录比“工作流变绿”更能证明流程可用。
