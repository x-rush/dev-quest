...
> **文档简介**: 用 Core Web Vitals、浏览器剖析和构建产物分析定位瓶颈，并把优化改动放入可重复的基线与回归检查中。本文的配置和采集片段需要按当前依赖与浏览器 API 校验后再接入项目。

> **目标读者**: 具备Next.js基础的中高级开发者，需要掌握性能优化和用户体验提升的性能工程师

> **前置知识**: Next.js、React、Web 性能指标、浏览器渲染原理和网络基础；具体 API 与版本以项目锁定依赖为准。
...
### ⚡ 性能优化核心技能
- 为 LCP、INP、CLS 等指标记录设备、网络、样本量、分位数和业务页面，而不是只记单次结果
- 区分构建耗时、网络传输、服务端等待、浏览器渲染和主线程阻塞，先归因再改动
- 根据实际渲染和缓存路径解释 Next.js 配置的影响，并用直接访问与客户端跳转复测
- 使用监控和剖析工具定位一次异常样本，保存证据并验证修复不会制造新回归

### 🚀 持续优化能力
- 为关键用户路径定义体验目标和可接受退化范围，而不以“高性能”作结论
- 设置有归属人、阈值和降噪规则的性能监控；告警必须能链接到可调查的样本
- 将资源大小、第三方脚本、请求数或体验分位数设为预算，并在 CI/发布前比较基线
- 设计 A/B 实验时区分性能指标、业务指标、样本偏差和停止条件

## 📖 概述

Next.js 的图片、路由、缓存和构建工具只能改变部分性能路径。真实体验还取决于接口响应、数据量、浏览器、网络、第三方脚本和用户设备。把“优化”写成一次假设检验：先记录基线，说明怀疑的瓶颈和仅修改的变量，再在相同条件下复测，并检查错误率、数据新鲜度和可访问性没有退化。

## 📊 Core Web Vitals 深度优化

### LCP (Largest Contentful Paint) 优化

> **示例边界**：下面的 `LCPOptimizer` 只说明性能观察器和上报层应如何隔离，并不是可直接复制的 LCP 采集器。`PerformanceEntry` 的字段、可观察的 entry type 以及归因方式以 [web-vitals](https://github.com/GoogleChrome/web-vitals) 和浏览器兼容性为准；不要用 `entry.name` 当作 CSS 选择器，或只因本地得到一次数值就宣称 LCP 已改善。
...
本章提供性能分析与优化的候选手段；是否改善真实用户体验取决于页面、设备、网络、缓存状态和数据依赖。每一次优化都应回到固定条件下的基线比较，不把采用某个构建器、预加载标签或监控 SDK 写成性能结论。

### 性能验收记录应包含

- 一个关键旅程及其 LCP、INP、CLS 或业务等待基线，注明设备、网络、缓存状态和样本窗口；FID 仅适用于历史数据，不能与 INP 混作同一指标。
- 一项可定位瓶颈和一次主要改动，附生产构建资源差异、前后指标及任何新增请求或内存代价。
- 对图片、字体、代码分割、缓存或预取的失败场景：慢网、缓存失效、旧权限、请求取消或资源加载失败。
- 与 release 和路由关联的回归记录，说明阈值、负责人、调查步骤和修复后复测结果。
