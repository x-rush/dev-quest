...
路由约定解决“请求落到哪里”，并不自动解决参数可信、资源授权或导航体验；这些边界要在每条关键路径中单独验证。
...
1. **目录约定对应用户状态**：为页面准备成功、加载、错误和找不到状态；共享 layout 只放跨子路由持续存在的结构，避免在根布局保存会随页面切换失效的局部状态。
2. **动态段来自 URL 文本**：解析前检查缺失、重复、格式和范围，资源读取后再检查当前身份是否有权访问。类型声明不能阻止手工构造 URL。
3. **路由组只整理源码**：组名不会出现在 URL；在引入前先确认它降低了导航或布局复杂度，不能把业务授权或数据隔离寄托在目录命名上。
4. **并行和拦截路由应有退路**：为直接访问、刷新、关闭浮层和错误分支分别设计可达页面。若一个模态只能从特定前序页面打开，刷新后应明确回到详情还是独立页面。
5. **导航可由键盘完成**：当前页有可感知标记，焦点在页面切换后落到有意义位置，面包屑使用实际路由层级而不是从显示标题反向猜测路径。

### 学习成果验收

- [ ] 访问一个嵌套路由，分别触发加载、错误、404 和成功状态，确认 layout 的共享部分与页面局部内容行为符合预期。
- [ ] 手工编辑动态段和查询参数，验证无效、重复、越界、资源不存在和无权限访问各有明确结果。
- [ ] 使用路由组重排源码后，确认公开 URL 没有改变，并说明组只承担的组织职责。
- [ ] 从列表打开一个拦截路由页面后刷新、复制链接、关闭浮层，确认每条路径都回到可解释的页面状态。
- [ ] 只用键盘完成导航，检查当前项语义、焦点移动和面包屑目标正确。
