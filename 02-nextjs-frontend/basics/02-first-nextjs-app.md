...
第一个应用的目标是建立可观察的开发闭环：能启动、能找到页面入口、能区分服务端和客户端交互、能从报错定位到文件，而不是只看到默认欢迎页。
...
1. **创建结果可复现**：记录所用包管理器和命令，启动开发服务器后访问本地 URL；若端口被占用、依赖未安装或运行时版本不兼容，先记录完整报错而不是随意修改配置文件。
2. **目录与 URL 能互相解释**：新建一个静态页面和一个动态路径，能从地址栏找到对应目录，也能说明 `layout` 与 `page` 分别承担共享结构和页面内容。
3. **客户端边界最小化**：默认组件保持服务端渲染；只有需要事件、浏览器 API 或本地状态的子组件使用客户端边界。不要把整个页面标记为客户端以逃避组件边界的理解。
4. **导航保留用户语义**：链接使用真实目的地和可访问名称；导航后能确认 URL、标题或主要内容变化，浏览器后退也回到预期状态。
5. **报错能定位与缩小**：主动制造一个语法、导入或渲染错误，使用终端和浏览器错误覆盖层找到文件与行号；恢复时只改与原因有关的最小部分。

### 学习成果验收

- [ ] 从空终端运行创建、安装和开发命令，保存运行时版本、启动 URL 与退出或错误输出。
- [ ] 新增一个 `/about` 页面和一个带参数的详情路径，直接访问、从链接进入和浏览器后退都得到可预期结果。
- [ ] 把一个点击计数器拆为最小客户端组件，确认父页面仍可作为服务端组件读取或组织内容。
- [ ] 只用键盘通过导航链接到达新页面，确认焦点与主要内容可感知地变化。
- [ ] 故意拼错一次导入或 JSX，记录定位信息；修复后确认页面恢复且没有通过隐藏错误或关闭检查来解决。
