...
选择数据获取方式时先写清数据在哪里读取、谁能看到凭据、何时会失效以及失败时用户看到什么；“能拿到 JSON”只是这个闭环的起点。
...
1. **服务端读取保护服务端能力**：在服务端组件或路由处理器读取时，可以使用服务端凭据并在返回前过滤字段；将需要浏览器交互的最小部分做成客户端组件，不要为了点击事件把整页的数据读取移到客户端。
2. **客户端读取处理生命周期**：客户端请求必须表现首次加载、空数据、首次失败、后台刷新与刷新失败。用户切换筛选条件或离开页面后，旧请求不能覆盖当前显示的数据。
3. **API 输入与输出都不可信**：路由处理器校验路径、查询和正文，再根据身份进行资源授权；客户端在使用响应前检查 HTTP 状态与数据形状。TypeScript 类型不能替代任一端的运行时检查。
4. **缓存是带条件的复用**：为每类数据写出缓存键、可接受陈旧时间、失效事件和刷新方式。写操作完成后，更新或失效对应读取；用户私有或权限敏感数据不得因共享缓存而串给其他用户。
5. **预取必须可取消且可衡量**：只对高概率的下一步预取，受网络、内存和优先级约束。通过命中率、额外请求量和用户可见等待时间判断它是否有效，不以“请求提前发出”当作优化成功。

### 学习成果验收

- [ ] 为一个页面标出服务端读取、客户端交互和敏感字段过滤的位置，确认浏览器响应不含服务端秘密字段。
- [ ] 快速切换两个筛选值并让前一个请求更慢，确认最终 UI 只显示最后一个筛选结果。
- [ ] 对 API 分别发送无效查询、无效正文、未登录和越权请求，确认每种情况都有明确状态码和不泄露资源的响应。
- [ ] 创建或编辑一条记录后返回列表，确认缓存没有显示旧数据；切换身份后确认不会复用另一用户的内容。
- [ ] 记录一次预取前后的命中、额外请求和等待时间，在网络受限时确认预取能被跳过或取消。
