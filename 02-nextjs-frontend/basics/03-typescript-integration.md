...
TypeScript 提高了重构和协作时发现错误的机会，但不会验证浏览器、URL、数据库或第三方服务送来的数据；每个跨边界值仍需要运行时检查。
...
1. **配置服务于明确约束**：开启严格检查后先修复真实错误，再逐项理解每个宽松选项的代价；不要为让旧代码通过而整体关闭严格模式或跳过库声明检查。
2. **领域类型避免复制**：从一个稳定的领域模型推导读取、创建和更新形状，区分服务器生成字段与客户端可写字段。接口和类型别名的选择以表达扩展关系或联合关系为准，不以风格争论代替模型设计。
3. **Props 表示完整状态空间**：为必需、可选和互斥 Props 选择对应的对象或判别联合；`forwardRef` 的 ref 类型与原生元素或公开实例相符，不能用 `any` 掩盖不兼容调用。
4. **API 类型必须配合运行时契约**：请求正文和响应在传输后不带 TypeScript 信息。服务端校验输入、返回稳定错误形状；客户端以状态码和解析结果处理成功、校验失败、未授权与服务故障。
5. **高级类型应减少重复而非增加谜语**：先写清调用方需要的输入与输出，再选择泛型、映射或条件类型。无法用一个小调用示例解释的类型工具，通常应拆成更具体的类型。

### 学习成果验收

- [ ] 运行 `tsc --noEmit`，故意引入一次空值、错误字段或不兼容 Props，确认错误信息能定位到调用点并在修复后消失。
- [ ] 为一个领域对象推导创建与更新 DTO，验证客户端不能写服务端 ID 或时间戳，空更新也会被运行时拒绝。
- [ ] 为组件设计互斥 Props，分别传入合法与冲突组合，确认冲突组合无法通过类型检查。
- [ ] 向 API 发送类型正确但业务非法的输入，以及未知字段，确认运行时验证仍给出明确错误。
- [ ] 为一个泛型工具写两个不同类型调用与预期返回类型，若无法表达，简化为更具体的函数。
