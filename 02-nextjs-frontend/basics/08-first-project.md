...
export async function POST(request: NextRequest) {
  try {
    ...
    // 本最小项目仅记录“待验证”状态：没有投递邮件，也没有签发验证令牌。
    // 因此响应不能声称用户已经收到验证邮件。
    return NextResponse.json({
      message: '注册已创建；本示例尚未实现验证邮件投递。',
      user,
      verification: { status: 'PENDING_IMPLEMENTATION' }
    }, { status: 201 })
...
上面的注册接口只负责创建用户记录，故意没有把“待验证”伪装成“验证邮件已发送”。它也**没有**限制待验证用户登录、生成验证链接、发送邮件或处理重试；这些是另一个可独立验收的交付范围。若产品需要邮箱验证，先写清楚待验证用户可否登录、可访问哪些资源，再实现以下闭环：

1. 为一次验证生成不可预测的随机令牌，只保存令牌摘要；令牌必须有过期时间和一次性使用标记。
2. 用可靠的事务外盒（outbox）或等价机制记录投递任务，避免“用户已创建但服务在投递前崩溃”造成永久收不到邮件；投递失败应可重试，响应中不能泄露邮箱是否已注册。
3. 验证接口必须同时检查令牌摘要、过期时间、是否已使用和所属用户，然后原子地把 `emailVerified` 置为 `true` 并作废令牌。
4. 用测试覆盖令牌过期、重复点击、错误令牌、投递失败重试，以及待验证用户访问受限资源的行为。只有这些场景有明确结果后，界面文案才可以说“验证邮件已发送”。
...
完成本章的功能和验收项后，你会得到一条可运行、可继续扩展的内容管理项目基线。它不自动包含邮件验证、支付、多租户、实时协作或生产运维；每一项扩展都需要单独定义数据边界、失败处理和验收场景。带着这里的路由、数据校验和权限分层继续完成下一项需求，并在提交前复现对应验收步骤。
