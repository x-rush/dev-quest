...
Tailwind CSS 4是功能优先的CSS框架，通过原子化类名提供快速、可定制的样式开发体验。与Next.js 16的深度集成，让现代Web应用的样式开发变得高效且一致。
...
样式工具只负责表达设计决策；可读性、可访问性、主题切换和构建体积仍需要在真实页面和实际交互中验证。
...
1. **令牌表达语义**：颜色、间距和圆角等常用值以用途命名，例如“危险操作”而非“红色 500”；组件变体从这些令牌组合，避免相似页面逐处复制不一致的类串。
2. **响应式从内容开始**：先在最窄可用宽度完成阅读、输入和操作，再为内容确实拥挤的断点调整布局。只拖动浏览器并看到元素没有溢出，不足以证明触控和文字缩放可用。
3. **主题切换不闪烁也不丢语义**：初始化主题前避免错误主题短暂显示；前景/背景、边框、焦点和状态色都要在每个主题中保持足够对比，不能只切换页面背景。
4. **变体输入受约束**：组件公开有限的 `size`、`tone`、`loading` 等变体；未知输入回退到安全样式或由类型与运行时校验拒绝，不能让任意类名绕过设计系统。
5. **性能从产物和体验测量**：检查生产构建的 CSS、关键页面加载与交互；不要仅根据“使用了 utility class”推断 CSS 更小或页面更快。

### 学习成果验收

- [ ] 为按钮、提示和表单错误定义语义令牌；更改一个令牌后，确认相关组件一致更新且无无关页面受影响。
- [ ] 在窄屏、宽屏、文字放大和键盘导航下完成关键表单，记录溢出、焦点与可读性结果。
- [ ] 切换浅色和深色主题并刷新页面，检查没有明显主题闪烁，错误、禁用、焦点和正文对比都可辨认。
- [ ] 传入一个未定义变体，确认类型检查或运行时回退能阻止不可预测的样式。
- [ ] 比较生产构建前后关键页面的 CSS 产物和用户可见加载结果，记录环境与测量条件。
