...
1. **从可访问行为开始**：通过角色、名称、标签和用户动作找到元素，断言用户能看到、读取或完成的结果。若只有 `data-testid` 才能测试，应先检查组件是否缺少可访问语义。
2. **给每个状态可观察出口**：加载、空数据、成功、权限拒绝与请求失败都要有稳定的文本或语义状态；测试等待这些状态出现或消失，不用固定毫秒数掩盖请求竞态。
3. **正确包住依赖**：Context、路由、主题和网络依赖应由最小测试 wrapper 提供。网络替身模拟协议边界而不是直接修改组件内部状态，避免测试绕过真实数据流。
4. **把 Mock 当成契约**：Mock 响应字段、状态码和错误形状要与服务端契约一致；改动接口时同步修改 mock，或加入契约检查，否则通过的组件测试可能掩盖集成失败。
5. **让失败有价值**：一次组件测试失败应能看出哪个用户动作、哪个期望状态或哪项依赖出了问题。覆盖率只提示遗漏路径，不能证明交互和可访问性正确。

### 学习成果验收

- [ ] 为一个组件覆盖加载、空、成功和失败状态，用可访问查询断言每个状态的用户可见结果。
- [ ] 在输入、点击或键盘操作后断言最终界面变化；故意删除一个交互处理器，确认测试会失败。
- [ ] 让 API mock 返回 401、422 和 500，确认组件分别显示适当的恢复路径，而不是把所有失败显示成同一句话。
- [ ] 在 Context 或路由依赖缺失时运行测试，确认 wrapper 只补充所需依赖，并没有掩盖生产环境必须提供的配置。
- [ ] 改动一个 mock 的字段名并运行类型检查或测试，确认接口不一致能被发现；恢复后记录通过结果。
