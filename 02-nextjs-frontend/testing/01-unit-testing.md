...
1. **测试边界**：单元测试验证一个函数、组件或 Hook 的可观察契约；浏览器导航、第三方系统和完整登录链路留给集成或 E2E 测试。测试金字塔是分配成本的依据，不是必须达到的覆盖率比例。
2. **用户可见行为**：用 Testing Library 按角色、标签和文本查找，再触发真实用户动作并断言可见结果；避免把组件内部 state、私有函数或 CSS 类名当作主要断言对象。
3. **异步与清理**：等待界面最终状态而不是固定 sleep；每个 mock、定时器、环境变量和网络处理器在用例间恢复，避免通过顺序才会出现的污染。
4. **依赖替身**：只替换测试不关心的边界（时间、网络、存储或昂贵服务），并为替身写出输入和返回约定；如果 mock 与真实接口形状不同，测试通过也不代表集成可用。
5. **覆盖率的用途**：把覆盖率报告作为“哪些路径尚未执行”的提示，针对权限拒绝、网络失败、空数据和重试等风险补测试；不以凑百分比替代断言质量。

### 学习成果验收

- [ ] 为一个表单或列表组件写出“初始、成功、失败”三个用户可见状态，运行测试并保存通过输出。
- [ ] 把一个断言故意改错，确认测试失败信息能指出预期行为；恢复后确认只依赖该用例的必要准备。
- [ ] 为一个异步 Hook 覆盖成功与失败，并验证卸载或重新请求后旧结果不会覆盖新界面。
- [ ] 让网络 mock 返回与真实接口不符的字段，确认类型检查、契约测试或组件测试能暴露问题。
- [ ] 查看覆盖率中未执行的分支，选择一个真实风险路径增加断言，并说明为何它比单纯提高行覆盖率有价值。
