...
1. **选择 E2E 的理由**：用 E2E 覆盖跨路由、浏览器能力或真实服务接缝中的少量关键旅程；把纯计算和大量边界条件留在更快的单元测试，避免所有行为都经由慢且易波动的浏览器测试。
2. **稳定定位与等待**：优先用角色、标签和可访问名称定位元素，等待 URL、响应或用户可见状态；固定等待时间只能掩盖竞态，不能证明页面已准备好。
3. **独立测试数据**：每个用例创建自己的账号、记录或命名空间，并在后置步骤清理；并行运行时不能依赖上一个用例生成的数据或默认排序。
4. **页面对象的边界**：页面对象封装重复操作和稳定选择器，测试本身保留业务意图。不要把整段断言和决策藏进对象，失败报告应仍能看出用户旅程在哪一步中断。
5. **失败可诊断**：CI 保存 trace、截图、视频（如项目允许）和服务日志关联信息；重试只能用于已识别的基础设施短暂故障，持续重试的失败应回到产品或测试同步问题排查。

### 学习成果验收

- [ ] 写一条从访问页面到完成关键操作的旅程，断言用户可见结果和 URL 或服务响应，而非仅断言元素存在。
- [ ] 让关键按钮短暂不可用，确认测试等待正确的可访问状态，而不是使用固定 sleep。
- [ ] 并行运行该用例两次，确认测试数据不互相覆盖，并在完成后清理创建的数据。
- [ ] 让其中一个业务断言故意失败，检查 CI 或本地报告是否保留足以定位的 trace、截图或步骤信息。
- [ ] 在 CI 中区分阻塞发布的关键旅程与允许单独重跑的非关键场景，记录各自的失败处理方式。
