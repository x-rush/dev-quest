...
开始前做一次自测：用数组渲染列表，用 props 传标题，用 useState 实现点击计数，并能解释 Promise 失败如何处理。缺哪项就先补该项，不要求先学完另一整套框架。

按下面顺序建立同一个练习工程。环境与依赖以[模块 README](README.md)的版本基线和项目锁文件为准；先完成页面和路由，再接数据与提交，缓存优化最后加入。官方安装入口：[Next.js App Router 安装说明](https://nextjs.org/docs/app/getting-started/installation)。
...
| 阶段与入口 | 练习输入与动作 | 通过条件 |
| --- | --- | --- |
| 页面与导航：[布局路由](basics/04-layouts-routing.md) | 做首页与详情页，刷新详情 URL，再输入不存在的路径 | 首页和详情均可直接访问；错误路径有明确结果；能指出共享布局 |
| 数据与交互：[数据获取](basics/06-data-fetching-basics.md)、[状态管理](basics/07-state-management.md) | 同页显示服务端列表和客户端计数器；分别提供空列表和失败响应 | 加载、空、成功、失败各有结果；按钮改变自己的状态，不把服务端秘密传入客户端 |
| 首项目：[浏览器文章草稿](basics/08-first-project.md#首次交付入口先完成浏览器文章草稿) | 读取已有草稿，新增 A/B，提交空白标题；刷新后再次读取，并注入一次无效存储内容 | A/B 从同站点的浏览器存储恢复；空白输入不增加记录；读取失败禁用保存且不覆盖旧值。交付页面、锁文件和四项实际结果；博客认证与数据库属于后续扩展 |
