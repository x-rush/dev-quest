# Next.js GraphQL + Apollo 集成：契约、缓存与验收

> **文档简介**: 以一个可演进的 GraphQL API 为例，说明 Schema、resolver、Apollo Client 缓存、订阅和权限各自解决什么问题；代码片段是集成骨架，必须按所选 Next.js 路由模式、Apollo 版本和服务端运行时补齐并验收。

> **目标读者**: 具备Next.js基础的高级开发者，需要构建现代API架构的后端工程师和全栈开发者

> **前置知识**: 已完成 Next.js 基础、了解 GraphQL 查询/变量/Schema、TypeScript、HTTP API 与数据库概念
...
### 🏢 企业级最佳实践
以一个“订单及明细”查询为单位设计 schema：字段含义、可空性、分页和权限都应明确。灵活查询同时带来成本风险，需要限制查询复杂度并检查 resolver 是否出现逐项访问数据库的 N+1。

用受控请求测试无权限字段、部分失败和分页边界，再记录耗时与错误位置。schema 变更先检查已有消费者是否兼容，文档生成只能展示结构，不能代替字段语义与安全规则的说明。

## 📖 概述

### 先决定问题是否适合 GraphQL

GraphQL 提供由客户端选择字段的单一查询入口；Apollo Client 提供查询执行、缓存和 React 集成。它们不会自动减少请求、避免 N+1、实现授权或提高响应速度。若资源关系简单、HTTP 缓存和标准状态码已足够，REST 往往更直接；当页面需要组合多种关联资源且团队能维护 Schema、resolver 成本与查询限制时，再评估 GraphQL。
...
|------|----------|------------------|
| **数据获取** | 为资源或用例设计多个 URL 与 HTTP 语义 | 客户端在一个操作中声明字段；resolver 仍可能访问多个服务 |
| **类型契约** | 可用 OpenAPI、JSON Schema 或手写约定 | Schema 可供校验与生成工具使用；业务语义仍需文档和测试 |
| **缓存** | 浏览器/CDN/服务端缓存可按 HTTP 设计 | 客户端可按 `__typename` 与 ID 标准化；分页与失效策略必须显式配置 |
| **实时更新** | 轮询、SSE 或 WebSocket | 订阅是协议能力；认证、断线恢复和事件去重仍需实现 |
| **演进** | 可加字段、弃用字段或引入版本 | 可加字段和标记弃用；删除或改变语义仍可能破坏消费者 |

#### 核心技术优势

**🚀 开发效率革命**
- 一个操作可声明所需字段，适合组合页面数据；是否减少网络往返取决于操作拆分、缓存和传输方式
- Schema 可配合代码生成提供类型检查与补全；生成类型不验证 resolver 的业务规则
- 声明式数据依赖便于集中管理查询；查询形状、缓存键、失效和预取仍须由团队设计
- 开发工具可查看操作与缓存；性能问题仍要测 resolver 调用次数、响应大小和真实网络耗时
...
本章的代码和模式用于理解 GraphQL 的请求、缓存和订阅边界；接入生产系统前仍需按实际身份模型、数据量、限流、监控与部署环境完成独立验证。
...
1. **Schema 是客户端与服务端的契约**：为查询、变更和错误结果定义稳定字段与空值语义。模式声明不自动授权；每个 resolver 仍要根据当前身份检查对象和字段是否可访问。
2. **变量不能被字符串拼接替代**：用户输入作为 GraphQL variables 传入，再在 resolver 中校验格式、范围和业务规则。限制查询深度、复杂度和分页大小，避免一条嵌套查询耗尽服务资源。
3. **resolver 要避免 N+1**：列表中每一项再查关联数据会随列表长度增加请求数。记录一次查询的调用次数，用批量加载或预取合并重复访问，并对真实数据量测量改动前后差异。
4. **客户端缓存依赖稳定身份**：为可缓存对象提供稳定的类型与 ID；变更后只更新、失效或重取真正受影响的字段。分页合并规则必须区分筛选条件，否则不同查询结果可能相互污染。
5. **订阅与错误是运行时协议**：连接断开、权限变化、重复事件和重连后漏事件都要有表现和恢复策略。GraphQL 的 `errors` 与网络错误分别处理，不能把部分数据响应一概显示成成功或失败。

### 学习成果验收

- [ ] 为一个资源写出查询、创建或更新变更及权限拒绝的错误形状；用两个身份验证 resolver 不会泄露他人字段。
- [ ] 对无效变量、超大分页和未知 ID 发起请求，确认服务端返回可预期错误且不会执行越界查询。
- [ ] 记录获取一个列表及其关联对象时的访问次数，加入批量加载后在相同输入下比较次数和响应结果。
- [ ] 创建或更新缓存对象后查看客户端缓存与页面，确认只有相关查询刷新，切换筛选条件也不会读到另一条件的数据。
- [ ] 模拟订阅断开、重连与权限撤销，确认界面显示恢复状态并不会继续应用未授权或重复事件。
...
**内容状态**: 本页说明的契约、缓存与验收清单已编写；文中的服务端、身份、订阅和部署片段必须在所选运行时与依赖版本中分别验收，不能由本文的代码展示推断为已部署或已通过集成测试。
**最后更新**: 2026年9月
**版本**: v1.0.0
