...
一次下单不是“购物车 + 支付按钮”的组合：服务端用当前价格和促销规则重新计算订单金额，创建待支付订单并保留库存；支付提供方的签名 webhook 只推进已存在订单的状态，不能由浏览器声明“已付款”；库存扣减、支付事件和发货任务各有可重试与幂等键。支付成功页只是用户体验，不能作为交付商品的事实来源。

购物车可在客户端提供快速反馈，但结算时必须重新验证商品是否可售、数量是否满足、价格和税费是否仍有效。库存策略要明确是预留、支付后扣减还是允许超卖，并写出取消、超时和支付失败如何释放资源。订单状态应是可追踪的状态机，例如 `pending_payment → paid → fulfilled/refunded/cancelled`，不允许任意字段更新跳过合法迁移。

搜索与推荐的结果必须与库存、价格、权限和分页契约一致；缓存可减少读取，但商品价格、库存或促销变化时必须定义失效方式。日志与审计记录订单 ID、支付事件 ID 和关联 ID，不记录完整卡号、支付密钥或可复用会话令牌。

### 学习成果检查

以下场景通过后，才说明项目的关键边界成立：

- [ ] 修改浏览器提交的商品单价、折扣或订单总额后，服务端仍按受信任商品数据重新计算并拒绝不一致请求。
- [ ] 同一个有效支付 webhook 重投两次，只会把同一订单从待支付推进到已支付一次；无效签名和未知订单不会写入状态。
- [ ] 两个并发结算请求争抢最后一件库存时，策略结果与文档一致，不会卖出负库存或产生两张成功订单。
- [ ] 支付失败、取消和超时分别释放或保留库存，且状态迁移、退款和用户可见提示可从订单审计记录追踪。
- [ ] 商品列表在价格/库存变更后不会长期展示错误的可购状态；分页、缓存键和失效条件均可由测试验证。
