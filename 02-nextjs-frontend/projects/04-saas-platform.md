...
把“企业级 SaaS”收敛为一个可追踪的租户生命周期：组织创建后拥有一个不可伪造的 `tenantId`；成员邀请和角色变更由授权规则控制；所有业务读取、写入、缓存键和异步任务都带该租户维度；订阅状态由支付提供方的已验证事件驱动，再决定可用功能和配额。不要从客户端传来的 tenant 字段直接信任归属，也不要把“当前套餐”只存在前端状态。
