...
**最小阶段**：先做单租户的一条受保护业务流程。

**验收结果**：未登录不能访问；两个租户的列表、详情和写入都隔离。

**扩展顺序**：计费、邀请、审计与多环境部署分开验收，不能用页面隐藏代替授权。
...
本篇是多租户 SaaS 的设计练习，以下是按职责选择的候选项，**不是必须同时安装的一揽子清单**。创建项目时让脚手架生成并锁定实际版本；本文不以可能变化的版本号或托管平台组合冒充通用基线。

| 职责 | 可选实现 | 选择条件与验收 |
|---|---|---|
| 页面、服务端渲染与路由 | Next.js + React + TypeScript | 先验证受保护路由、服务端身份边界和错误页；框架升级前在锁定依赖的候选工程构建 |
| UI 与本地交互状态 | 项目组件、Tailwind，必要时使用 Zustand | 服务端数据不复制为长期本地真相；切换租户时本地草稿、主题和权限状态都有明确清理规则 |
| 服务端数据缓存 | TanStack Query 或 Next 的数据获取机制 | 查询键包含租户、主体和筛选；登出／租户切换后不展示旧身份缓存 |
| 数据库和 ORM | PostgreSQL；Prisma 或显式 SQL | 强制租户过滤、事务和迁移演练；ORM 不代替索引、授权和数据库备份 |
| 认证、支付、邮件、实时通信、对象存储 | 选择能满足地区、合规、预算和运维约束的供应商 | 分别验证账号隔离、webhook 签名、邮件退信、连接恢复和上传权限；不要因为示例出现某个品牌就提交其凭据 |
| 监控和部署 | 团队已有平台或受管理服务 | 构建产物能追到源码提交；告警有负责人、阈值和降噪；供应商迁移不改变业务权限边界 |
...
| 原则 | 落地动作 | 可检查结果 |
|---|---|---|
| 一致性 | 将间距、颜色、状态提示和危险操作确认收敛为可复用组件；新页面复用同一加载、空态、失败态 | 在两个不同特性页完成相同删除操作，焦点、确认、成功／失败反馈和键盘路径一致 |
| 可访问性 | 为表单提供可关联的 label 与错误文本；弹窗管理焦点；颜色之外再用文字或图标表达状态 | 用键盘完成登录、筛选、保存和关闭弹窗；错误信息能被辅助技术读到，图表有文字摘要 |
| 国际化 | 将用户可见文本、日期、数字和货币从业务逻辑中分离；租户时区和 locale 作为数据边界 | 切换 locale 后日期、金额和验证消息正确，存储的时间仍使用统一时刻而非格式化字符串 |
| 可定制性 | 只允许租户在受控 token 范围内改品牌色、logo 和主题，避免把任意 CSS／脚本注入页面 | 切换租户后主题正确且不会污染另一租户；无效 token 回退到默认主题 |
...
把“企业级 SaaS”收敛为一个可追踪的租户生命周期：组织创建后拥有一个不可伪造的 `tenantId`；成员邀请和角色变更由授权规则控制；所有业务读取、写入、缓存键和异步任务都带该租户维度；订阅状态由支付提供方经签名校验的事件驱动，再决定可用功能和配额。不要从客户端传来的 tenant 字段直接信任归属，也不要把“当前套餐”只存在前端状态。

订阅计费的关键是幂等和状态机，而不是“调用支付 API 成功”。为每个支付事件保存提供方事件 ID 并在事务内更新订阅；重复 webhook 不应重复扣配额或重复发邮件。事件签名校验失败、事件乱序、取消后仍有宽限期、升级/降级在何时生效，都必须有产品规则和可重放的测试输入。

权限设计从资源动作开始，例如 `project:read`、`project:write`、`billing:manage`，再把角色映射到动作。服务端在每个资源操作上同时检查身份、成员资格、角色和资源所属 tenant；UI 只负责减少无效点击，不能代替 API 授权。白标定制也必须由可信租户配置决定，主题、域名和品牌资源不能成为跨租户数据读取的入口。

性能与可观测性首先保证可定位：请求日志与指标带 tenant、路由、状态码和关联 ID，但不记录令牌和支付数据；为列表查询设页大小、排序和索引；为每个租户及全局设置速率、并发和存储配额。发现慢请求后先区分 N+1 查询、缺索引、外部服务等待和冷缓存，再改变实现，不用“加缓存”掩盖权限或一致性错误。

### 学习成果检查

用以下验收场景替代“功能看起来齐全”的自评：

- [ ] 租户 A 创建项目后，租户 B 的成员即使猜到项目 ID 也无法读取、更新或从缓存得到该项目；数据库查询和异步任务记录都能显示 tenant 过滤。
- [ ] 同一合法支付 webhook 投递两次只改变订阅一次；篡改签名的请求被拒绝且不写订阅状态；乱序事件按文档化的状态机处理。
- [ ] 成员被移除或角色降级后，下一次服务器请求立即失去对应动作权限；前端隐藏按钮不是唯一防线。
- [ ] 选择高级套餐与超出配额各有明确响应、审计记录和恢复路径；配额计数不可因为并发请求被绕过。
- [ ] 一次慢列表请求可用关联 ID 追到路由、租户、数据库查询和外部调用；日志、错误响应和监控事件均不泄露令牌、完整支付信息或其他租户资料。
