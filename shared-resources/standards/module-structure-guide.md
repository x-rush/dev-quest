...
**核心原则：字典无等级，学习有路径。** 同一个概念在 `reference/` 中只有一份权威条目，新手和专家查的是同一处；而学习过程按难度路径穿行各目录。
...
### 内容差异化原则
- **单一事实来源**: 一个概念只在一处有完整解释（`reference/`），其他位置链接过去
...
- 注重学习过程和练习，教程式而非参考式
- 提供渐进式的代码示例
- 概念的完整解释交给 `reference/`，basics 中链接过去即可，不重复展开
