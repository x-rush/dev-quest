...
本库持续增强覆盖 **11 个技术模块**。最新结构基线扫描到 682 篇 Markdown，其中 644 篇是这 11 个模块中的活跃技术文档；其余为共享标准、工具说明和历史/生成记录。面向的读者是“会基本编程，但不熟悉对应技术栈”。历史重构档案作为过程记录保留。
...
| 入口 | 能看到什么 |
|---|---|
| [逐文件台账](reports/coverage.md) | 11 个模块 644 篇活跃技术文档的处理层级与结构复核线索 |
...
| 活跃/共享文档章节锚点 | 543 个引用，静态检查 0 缺失 | 已在所有 Markdown 渲染器点击验证 |
...
实际解释器、依赖和系统镜像版本以各运行报告的 JSON 为准；它们是带时间戳的证据，不应被这里的摘要误读为长期固定基线。TypeScript 的类型测试不能替代模块拟采用版本的完整工程构建。Swift、Kotlin/Android、Java、PHP、Rust 的完整平台工程未在本机执行；Go 的早期 Windows 阻断记录不再被当成通过，本轮可运行标准库示例改由离线 Go 容器实际验证，见 [Go 离线运行验证](reports/go-runtime-validation.md)。
