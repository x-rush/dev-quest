...
CASES = (
    ...
    ("go-di-register-contract", "01-go-backend/reference/language-concepts/03-go-programming-essentials.md", "email=ada@example.com first=true duplicate=true\n"),
    ...
    ("go-sync-mutex-waitgroup-once", "01-go-backend/reference/library-guides/06-sync.md", "hits=100 once=1 atomic=5\n"),
    ("go-concurrency-collect-close", "01-go-backend/reference/language-concepts/08-concurrency-basics.md", "[10 20 30]\n"),
)
...
def main() -> None:
    ...
    report = {
        ...
        "purpose": "Twenty-fifth-round runtime evidence for eighteen marked Go core-document fences.",
        ...
        "scope": "Only the eighteen named complete Go fences extracted unchanged after CRLF-to-LF normalization were executed. Other fences, SDK installation, toolchain setup, web services, filesystem integrations, concurrency scheduling, performance, and projects remain outside this evidence.",
    ...
    lines = ["# Go 核心基础页第二十五批运行验证", "", "仅运行下列十八个正文中有 `doc-verify` 标记的完整 Go 围栏。报告不将结果扩大为整页、SDK 安装、工具链或项目验证。", "", "| 文档 | 示例 | 结果 |", "|---|---|---|"]
