...
回滚实验白名单后，用相同输入重新测量：先确认界面仍展示预期数据和错误状态，再比较提交成本；两项观察都符合预期时才保留优化。函数引用稳定可避免不必要的 `select` 重新计算，但不表示组件不会再次渲染。完成后再进入[渲染性能](../../advanced-topics/performance/02-rendering-performance.md)分析昂贵子组件。本轮只核对官方资料并静态审阅，未运行 TypeScript 编译、Profiler 或性能基准；请在自己的项目中记录测量结果后再作判断。
