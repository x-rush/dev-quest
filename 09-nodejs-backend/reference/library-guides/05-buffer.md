...
### 定义
`Buffer` 是 `Uint8Array` 的**子类**，附加了 Node 特有的编码转换与读写方法；Web 标准 API（fetch、Blob、WebCrypto）只认 `Uint8Array`/`ArrayBuffer`，边界处用 `Buffer` 的视图能力自然衔接。
...
Buffer.from(u8arr.buffer);     // 从 ArrayBuffer/TypedArray（零拷贝视图）
```
...
| `from(data)` | 数据拷贝 | 取决于源 | 从字符串/数组/Buffer 转换 |
...
b.readUInt16BE(2);            // 越界或截断偏移直接抛 RangeError
```
