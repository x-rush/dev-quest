...
先写明集成范围：验证路由与服务可以替换数据库，验证事务与查询就需要真实数据库语义。不能只凭用了 mock 判定测试名称，而应明确哪些缺陷本测试有能力发现。

每条用例建立独立数据并通过 API 验证状态码、契约字段与持久化效果。业务规定的错误码或文案也可以断言，避免不必要依赖文本排版即可。换序和单独运行都应得到相同结果。
...
使用独立测试数据库启动路由与服务，创建记录后通过查询接口读回，再请求一个不存在的 ID。预期成功数据契约与 404 契约都正确，数据库约束错误不会泄露 SQL 或秘密配置。验收故意在保存阶段失败，确认没有对外报告成功；测试结束关闭服务器和连接池，进程不因遗留句柄卡住。
