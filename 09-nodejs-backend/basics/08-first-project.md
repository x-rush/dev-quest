...
最后得到不存在；非法输入不创建记录。本页基线使用 SQLite，另建一条未删除的记录，重启服务后仍应查回。
...
> **前置知识**: [环境搭建](./01-environment-setup.md)、[ESM 模块](./03-modules-esm.md)、[异步与 Promise](./04-async-promises.md)、[路由与中间件](./05-http-routing.md)、[错误处理](./06-error-handling.md)。流与 Worker Threads 是后面的挑战项，不要求先完成。

本课产物是包含 Prisma 迁移、生成客户端、任务路由和测试的 SQLite API 工程。完成正文的类型检查、测试库迁移与 4 项测试后，恢复开发库配置，再启动服务执行“创建 → 查回 → 更新 → 删除 → 再查为 404”；另外保留一条记录验证重启后仍能读取。保存实际命令、响应和测试结果，正文中的预期值不能代替本地验收。

类型或导入失败先回查 ESM 配置与客户端生成步骤；数据库表不存在先核对迁移和当前 `DATABASE_URL`；请求变成 500 则回查错误出口；重启丢数据先核对进程是否使用同一个数据库路径。通过后进入 [Todo API 扩展](../projects/01-todo-api.md)，沿用本工程新增 Todo 模型和独立路由，保留 Task 与原有测试作为回归对照。
