...
**最小阶段**：把一条完整业务流程放入发行环境并保留诊断。

**验收结果**：启动失败可定位配置，健康检查反映状态，数据库升级有恢复思路。

**扩展顺序**：安全、性能、弹性与监控分别通过实验验证，不用框架数量表示成熟度。
...
在隔离环境模拟下游超时，验证服务是否按预算失败、释放资源并给出可理解响应。重试只用于允许重试的操作，设次数、总时限与退避，避免每层都重试把一次失败放大。

观察订单成功率等业务指标与 JVM、连接池指标之间的关系。配置变更应可追溯且有恢复步骤，日志只保留必要并脱敏的上下文；“系统资源正常”不能证明用户任务成功。
