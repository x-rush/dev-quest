...
集成测试先声明保留哪些真实组件，例如 HTTP、事务与数据库，外部支付可用沙箱或受控替身。测试命名后缀要与构建插件配置一致，否则文件存在也可能从未执行。

每条测试创建自己的数据并使用返回 id，等待异步结果时给出条件与上限。共享容器可以节省启动成本，但必须隔离数据；同一用例应能单独与整套运行都通过。
...
创建独立数据库环境并应用正式迁移。写入一条实体后再次写入违反唯一约束的数据，预期获得正确的异常分类；再在一个事务内先写入后抛错，事务外查询不应出现残留。验收要区分测试框架自动回滚与业务事务回滚：如果所有断言都在测试自己的事务中，可能掩盖真实提交边界。重复和单独运行均不能依赖执行顺序。
