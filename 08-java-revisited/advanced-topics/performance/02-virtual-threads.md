...
生效后：每个 HTTP 请求由一个虚拟线程处理；`@Async`、`@Scheduled` 也自动切换。**前提**：Java 21+（推荐 25）+ Boot 3.2+（Boot 4 沿用同一属性）。
...
2. **ThreadLocal 谨慎用**——百万线程 × 大对象 = 堆膨胀；跨线程共享数据用 `ScopedValue`（Java 25 已转正，JEP 506）
