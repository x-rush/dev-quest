...
Cargo 把源码、依赖、测试与构建配置组织起来。单测靠近内部逻辑，集成测试从公开 API 使用 crate，两者观察边界不同。

**本节自测**：给纯函数写正常与失败测试，再从 tests 目录调用公开 API。

<details>
<summary>预期结果与参考思路（先尝试再展开）</summary>

测试既检查输出也检查错误；不能用 cargo build 成功替代业务行为验证。
...
锁文件固定本工程的依赖解析，工具链与目标环境也参与可复现构建。测试名说明输入条件与预期，公共 API 的可执行示例帮助同时维护用法和行为；不能为保持绿灯无理由忽略失败示例。

profile 管理优化与相关构建设置，其中某些设置会影响可观察行为；feature 用于能力选择，也不只限于依赖开关。声明支持的组合并在 CI 验证，避免承诺所有组合都可用却只测试默认值。
