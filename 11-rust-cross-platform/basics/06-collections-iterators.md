...
- **`String`**：拥有、可增长且始终有效 UTF-8 的文本类型；可以通过 `into_bytes` 取得字节向量，但不应依赖其内部表示。不支持 `s[i]` 下标取字符（字节偏移 ≠ 字符序号）
...
- **零抽象成本**：迭代器适配器以静态类型组合，通常可优化为与直接循环接近的代码；是否内联及最终性能仍取决于具体代码、编译选项和测量结果
