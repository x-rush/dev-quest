...
struct 表达同时存在的字段，enum 表达互斥的状态。把状态建模进类型，可以避免多个布尔值组合出无效情况。

**本节自测**：用 enum 表达加载、成功、失败，match 出展示文本。

<details>
<summary>预期结果与参考思路（先尝试再展开）</summary>

每种状态都有处理；成功数据只出现在成功分支，不需要对全局可空字段反复猜测。
...
enum 表达互斥状态，可避免 loading 与 failed 同时为真；多个可独立成立的事实仍可用多个布尔值。match 的分支结果需要能统一到兼容类型，某些发散分支如提前返回有不同推断行为，不能只按表面写法判断。
