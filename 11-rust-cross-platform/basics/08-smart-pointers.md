...
**检查方法**: 在项目中运行示例六的 `should_panic` 测试，并核对它确实因预期原因 panic。
...
**检查方法**: 让 10 个线程各加 1，并核对本次运行的结果为 10；这只能检查该示例，不能取代并发设计审查。
...
**A**: 它们都允许在共享引用下改变值，但解决的约束不同。`RefCell<T>` 用运行时借用标志执行单线程的借用规则：冲突借用会 panic；`Mutex<T>` 用互斥保护跨线程访问：竞争时会等待，错误的锁顺序、重入或 guard 生命周期可能造成死锁。`Mutex` 的实现是否进入内核取决于竞争与平台，不能据此推断固定性能。`Rc<RefCell<T>>` 与 `Arc<Mutex<T>>` 的类型形状相似，却不能机械替换：迁移后要重新审查 `Send`/`Sync`、锁粒度、等待期间是否持锁，以及错误与取消路径。
