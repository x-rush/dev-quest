...
E2E 通过真实窗口完成核心旅程，等待元素与状态条件，并保存截图、应用日志和驱动版本。平台要求虚拟显示或特定 WebDriver 版本时，将其作为可复现环境的一部分记录。

调试构建便于诊断，发布前仍需要对实际 release 产物冒烟或适当的 E2E 验证，不能因符号被剥离就禁止测试它。算法细节在更快测试层验证，端到端重点检查打包、权限、IPC 与界面能否协作。
