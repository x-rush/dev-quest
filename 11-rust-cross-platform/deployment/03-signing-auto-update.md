...
**定义**: tauri 更新器采用 minisign 风格的签名验证——**私钥离线签产物，公钥内嵌进应用**，客户端只接受能由对应公钥验签的更新包；托管服务器是否被入侵并不改变签名验证的信任边界。
...
更新签名绑定具体产物，重新构建后必须重新签名并验证客户端下载与校验路径。私钥放在受控秘密存储，限制使用权限并准备备份、恢复与轮换；不能用“立即删掉唯一副本”换取表面安全。

已发布客户端内置的公钥决定它信任什么，轮换需设计旧客户端能验证的迁移路径。先在受控发布渠道检查各平台的下载、验签和安装，故意使用不匹配签名应被拒绝。草稿或分阶段发布有助审核，但不能替代兼容性测试。
