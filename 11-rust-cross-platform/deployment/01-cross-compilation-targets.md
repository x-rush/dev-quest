...
区分交叉编译、链接、打包、签名和运行验证：某目标能生成二进制，不代表当前主机能完成该平台所有发布步骤。按实际支持矩阵选择原生 runner 或交叉工具，记录系统库和 SDK 要求。

gnu/musl 选择要匹配运行环境和 C 依赖，不因追求小镜像就强行切换。旧系统上的动态库版本可能限制新环境构建的产物；在目标环境运行冒烟测试，确认网络、证书和文件能力，而不只检查编译退出码。
