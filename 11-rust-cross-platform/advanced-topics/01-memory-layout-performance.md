...
> **文档简介**: 用 `size_of`/`align_of` 实测数据拆解 Rust 的内存布局规则——repr(Rust) 的重排自由、repr(C) 的确定性、栈堆句柄的真实尺寸、String/&str/Cow 的分配取舍，以及迭代器零成本抽象的实证视角
>
...
> 本文所有实测输出来自 `rustc --edition 2024`（基线见模块 README，Rust 1.98.1），本机为 x86_64 Linux；对齐与尺寸在主流 64 位平台上一致。
...
**实测输出**: `align = 8, size = 16`

**关键点解析**:

- 对齐 = 最大字段对齐 = 8；先按对齐心算一遍，再用输出验证——这个习惯比背结论更抗过时
- 默认 repr(Rust) 下编译器已把字段重排塞紧（16 字节）；若要按声明顺序发给 C，加 `#[repr(C)]` 则为 24 字节（对照第一节实测）
...
- **热点结构先量再改**: `size_of` 一行命令就能证伪「我觉得它很大」的直觉
- **减少堆跳转优先于减少字节数**: 缓存未命中的代价通常远超 padding 损失；`Vec<T>` 换 `Box<[T]>` 能省掉容量字段，但先确认它真是热点
- **写时克隆用 `Cow`**: 参数与返回值「多数不改、偶尔要改」的 API 是 `Cow` 的标准应用面

### ❌ 避免陷阱

- **把 repr(Rust) 的布局写进二进制协议**: 字段顺序与大小随编译器版本都可能变——跨界一律 `repr(C)`
- **`#[repr(packed)]` 后直接取字段引用**: 未对齐引用是未定义行为，packed 结构体内字段一律按字节拷贝访问
- **在 debug 构建下下性能结论**: 所有本篇的耗时数字都来自 `-O`；debug 数字只反映解释器般的执行形态
...
**评估标准**: 手算与实测一致；重排后比原声明少一个数量级的填充字节。
