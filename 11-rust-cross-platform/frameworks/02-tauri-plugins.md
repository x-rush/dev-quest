...
**Tauri 插件**：插件扩展原生能力，权限配置限制可调用范围。安装依赖、注册插件、授权调用与平台支持是不同步骤。

**最小练习与预期结果**：接入一个能力并先故意不授予权限，确认被拒绝；再只增加所需权限，成功后核对平台支持。
...
插件减少平台适配工作，仍要核对所用版本、支持平台与权限范围。只需要打开网页时，不要额外授予任意执行命令；文件能力限制到业务所需路径并测试越界拒绝。

更新公钥用于验证，私钥用于签名，两者责任不同。按配置契约填写内容与作用域，重建后验证新权限确实生效。官方来源不等于无需审查，新增插件仍需评估依赖、数据访问与失败行为。
