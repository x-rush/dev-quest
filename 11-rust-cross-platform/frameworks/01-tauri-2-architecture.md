...
**Tauri 架构**：Web 前端与 Rust 命令通过 IPC 交换可序列化消息。前端发出命令不代表已获得任意系统权限，边界还需要能力与输入约束。

**最小练习与预期结果**：做一个接收文本并返回长度的命令，验证正常与非法输入；画出参数经过前端、IPC、Rust 再返回的路径。
...
IPC 是从界面到原生能力的入口，命令应只暴露完成任务所需的操作，并在 Rust 侧验证输入、路径与授权。把秘密移进 Rust 可以缩小暴露面，但桌面二进制仍可能被分析，服务秘密不能仅靠换语言隐藏。

大文件尽量在原生侧处理，前端取得有限结果或进度；async 关键字不自动移走阻塞工作，要安排合适执行资源。用目标系统验证 WebView 行为与权限，并记录生成文件如何从配置重建。
