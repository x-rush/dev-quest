...
**SQLx 与共享状态**：Arc 共享所有权，锁保护特定可变操作，连接池管理数据库连接。SQL 编译检查需要相应元数据，但不证明事务与业务语义正确。

**最小练习与预期结果**：执行一条参数化查询并模拟事务失败，检查回滚；不要跨 await 持有不必要的阻塞锁。
...
应用按配置共享连接池，最大连接数要考虑所有副本总量及数据库容量；多租户隔离等需求可能需要多个池，不存在永远全局唯一的规则。离线 SQL 元数据要随 schema 更新并在 CI 检查，旧缓存不能证明新数据库兼容。

锁只保护所需不变量，尽量在临界区内取得必要值、锁外进行慢 I/O。改用异步锁不会消除串行等待；参数值使用绑定，动态列名等不能绑定的位置使用受控白名单。用并发操作测试事务与共享状态一致。
