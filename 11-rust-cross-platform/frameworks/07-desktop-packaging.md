...
**Tauri 打包**：开发启动成功、原生编译成功、安装包可安装和签名可信是四个阶段。平台系统依赖可能阻止在一台机器上构建全部目标。

**最小练习与预期结果**：选一个目标平台从干净工程构建，安装后走通核心命令；记录架构、签名和数据目录行为。
...
先列出发布渠道真正需要的安装格式，再选择工具明确支持的主机与目标组合；原生 runner 常简化签名与平台依赖，但不能概括所有跨平台打包都不可能。只生成所需格式可以减少构建工作。

找不到安装包时检查 bundler 开关、输出目录和实际构建命令，不凭固定默认值猜原因。归档版本、图标与签名信息，使用干净目标系统验证安装、启动和卸载，并检查用户数据的保留策略。
