...
**最小阶段**：先用一个路由读取数据库记录，再补创建。

**验收结果**：无效输入与未知 ID 分开响应；数据库异常不直接暴露内部细节。

**扩展顺序**：测试迁移与约束，再加入认证、分页和连接池调优。
...
请求模型只包含客户端允许填写的字段，服务端生成 id 和时间等受控值；输入格式正确后仍要验证权限与业务不变量。内部错误保留原因，在 HTTP 边界转换成稳定状态和安全消息。

显式查询列减少不需要的数据并使映射可见，但 schema 变更仍可能破坏契约。共享连接池并结束事务，验证不存在、冲突与数据库失败三条路径。Axum 0.8 路由捕获按版本语法定义，并测试实际路径匹配。
