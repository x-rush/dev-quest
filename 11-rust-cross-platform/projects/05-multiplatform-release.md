...
**最小阶段**：先在一台目标机器构建并运行一个产物。

**验收结果**：从干净环境重现构建，产物架构与目标系统一致。

**扩展顺序**：CLI、桌面与服务端分别测试；签名、更新与回退各有独立证据。
...
发布流程让版本、提交和每个平台产物互相可追溯；多个文件保存版本时可以由工具同步并检查一致性，不必要求物理上只能出现一次。锁文件帮助固定解析，系统依赖和签名环境仍要单独管理。

发布草稿先验证安装与升级，生成文件通过源配置修改以避免下次覆盖。服务器与客户端可能独立版本演进，应声明兼容范围，不强行伪装同一版本。计划检查与本地构建能提前发现问题，但不能承诺拦住固定百分比缺陷。
