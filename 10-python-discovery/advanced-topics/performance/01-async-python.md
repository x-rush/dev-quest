...
- `await` 只在"等待 IO"时产生并发；两个 `await` 之间的同步代码段仍独占线程，不会被打断——所以协程内读改共享状态通常不需要锁
...
**红线**：`async def` 里调用阻塞函数（`time.sleep`、同步 DB 驱动、`requests`）会**卡住整个事件循环**，所有并发归零、全部请求排队。
...
async def crunch_numbers(data: bytes) -> int:
    ...
    return await loop.run_in_executor(None, blocking_hash, data)
```

**要点**：事件循环只解决 IO 并发。图像处理、加密、大计算交给 `run_in_executor`（进程池）或独立任务队列（Celery/ARQ）。
...
- [ ] 分层架构中异步只出现在 IO 层（与[项目分层](../architecture/01-project-architecture.md)协同）
