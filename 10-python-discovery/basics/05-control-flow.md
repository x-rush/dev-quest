...
推导式是建立新容器的表达式，普通循环更适合多步骤副作用。生成器表达式则延迟生产，不是另一种已经填满的列表。

**本节自测**：筛出偶数并平方，输入空列表，再把列表推导式换生成器并消费两次。

<details>
<summary>预期结果与参考思路（先尝试再展开）</summary>

空输入得到空结果；生成器耗尽后再次消费为空，若需复用要重新创建或保存结果。
...
需要索引和值时用 enumerate，需要键值时用 items，可使循环目的更明显；确实要按位置修改或访问邻居时，索引循环仍然合理。推导式适合短的变换，复杂条件和副作用用普通循环更易追踪。

生成器避免提前保存所有中间项，但只消费一次且最终聚合仍可能需要内存。for-else 表示循环未被 break 终止，不是“循环条件为假才开始执行”。用找到、未找到和空输入三个例子验证理解。
