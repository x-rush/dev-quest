...
**实例属性 → 类属性 → 沿 MRO 逐层父类**。`a.name = "x"` 只写实例层，不碰类；实例属性被 `del` 后回退到类属性。本机 3.14.7 实测：`k.x = 2` 后 `k.x` 为 2 而 `K().x` 仍为 1；`del k.x` 再读回退到类属性 1。
...
一切类的终点。常用兜底实现：`__init__`（无参）、`__repr__`、`__eq__`（默认身份比较）、`__hash__`（默认按 id）、`__str__`（默认同 `__repr__`）。覆写 `__eq__` 时记得同步考虑 `__hash__`（详见 [04-oop-protocols](./04-oop-protocols.md)）。
...
- ❌ **子类重写 `__init__` 忘调 `super().__init__()`**：基类负责的字段全部缺失。
  ✅ 初始化首行写 `super().__init__(...)`，参数按协作式约定透传。
