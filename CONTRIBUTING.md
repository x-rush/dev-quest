...
知识库同时维护基础知识全量参考、从 0 到 1 的路径、高级扩展和有长期价值的框架学习。首先遵循[写作与验收规范](shared-resources/standards/documentation-guidelines.md)：关键词、内置函数与常用标准库需要解释输入输出、例子与边界；教程应就地讲清当前步骤，不只链接字典。

优先补知识和反馈缺口，不以统一标题、堆砌术语或增加篇数代替增强。参考词条的覆盖率与示例正确性分别核查；未运行的平台不要写“实测通过”。
