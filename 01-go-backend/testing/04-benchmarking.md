...
- **测量循环**: 自动调整迭代次数并报告每次操作指标；跨次统计比较仍需重复运行与分析工具
- **内存分析**: 支持内存分配和GC性能分析
...
1. **比较同一语义**：两种实现必须处理相同输入并产生同样结果；准备成本是否计入计时要对双方一致，否则 `ns/op` 没有可比性。
2. **让计时区只包含目标工作**：在循环外构造固定夹具，循环内执行被测操作并保留可观察结果，避免编译器移除计算。正确性由普通测试覆盖，基准不承担所有功能断言。
3. **把波动当作数据**：固定 Go 版本、输入规模和尽量稳定的机器条件，多次执行并保留原始输出；不从一次最快结果得出优化结论。
4. **指标要与问题匹配**：`ns/op`、`B/op` 和 `allocs/op` 描述局部代码。服务端吞吐、错误率和 p95/p99 延迟还需要负载测试，不能把微基准直接外推到线上 SLA。

### 学习成果验收

- [ ] 为两个等价实现写正确性测试和同一输入规模的基准，说明准备步骤是否计时。
- [ ] 运行多次 `go test -run=^$ -bench=. -benchmem -count=5`，保存命令、Go 版本、机器条件和全部结果。
- [ ] 对一个明显无副作用的计算加入结果保留或断言，确认基准没有被优化成空工作。
- [ ] 用统计比较工具或完整原始结果判断差异是否稳定，并说明分配、延迟或内存哪项实际改善。
- [ ] 若结论要用于 HTTP 服务，再补负载测试中的吞吐、错误率和分位延迟，而不是只引用微基准数字。
...
前置是测试函数与基本统计。ns/op 是每次操作平均耗时，B/op 和 allocs/op 表示每次操作的分配量与次数，均不直接代表整个服务的尾延迟。比较实现时固定输入规模与结果语义，避免一边把准备数据算进计时，另一边排除。编译器可能移除无可观察效果的计算；适用版本可使用 b.Loop，并在正确性测试中另行验证输出。

运行 go test -run=^$ -bench=. -benchmem -count=5 保存多次结果，比较波动而非只挑最快一次。基准中的缓存状态、CPU 频率、并发任务、Go 版本都会影响结果。想评价 HTTP 服务，还需负载测试报告请求量、错误率和延迟分布；把空函数的纳秒差异外推成线上性能提升没有依据。

练习：用 10、1000、100000 个元素分别比较线性查找与 map 查询，同时明确构建 map 的成本是否计入。如果只查询一次，构建 map 未必划算；重复查询许多次才可能摊薄准备成本。验收报告要写输入、运行环境、命令、多次结果和正确性检查，再解释收益适用的范围。
