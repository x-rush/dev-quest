...
1. **先说明要隔离什么**：Stub 固定返回值以覆盖业务分支，Fake 提供可工作的简化实现，Mock 验证必要交互。选择替身时写清被隔离的是时间、网络、存储还是通知；工具名称不是测试目标。
2. **接口按调用方需要设计**：在业务层依赖小接口，在组合根注入实现。不要为生成 Mock 暴露整套数据库或 HTTP 客户端 API，否则测试会绑定大量无关交互细节。
3. **断言结果优先于实现过程**：能从返回值、状态变化或可见副作用验证的规则，不应只断言“调用了某个私有方法”。确需 Mock 交互时，断言次数、参数与禁止的重复调用。
4. **真实接缝另有验证**：Fake 和 Mock 不覆盖 SQL 约束、事务、序列化、TLS 或网络超时。为每个外部适配器保留少量集成测试，验证它与真实协议或临时依赖的契约。

### 学习成果验收

- [ ] 为同一个业务规则分别选用 Stub 或 Fake，说明为何该替身足以覆盖目标分支。
- [ ] 把业务函数从具体存储依赖改为最小接口，确认生产实现与测试替身都能满足它。
- [ ] 让通知 Mock 被错误地调用两次，确认测试能失败并指出重复副作用。
- [ ] 让 Fake 与真实数据库唯一约束产生不同结果，补一条集成测试证明真实接缝的行为。
- [ ] 修改接口后重新生成或更新替身，确认编译和语义断言都通过，而非仅保留旧生成文件。
