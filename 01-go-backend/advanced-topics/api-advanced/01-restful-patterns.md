...
完成本模块后，你应能交付一个可复查的资源接口契约：

1. 为一个资源写出方法、路径、请求字段、成功响应、非法输入、缺失资源与权限不足的响应；同一错误语义在所有 endpoint 使用稳定 code，而不是只写“统一处理”。
2. 为分页、筛选和排序定义白名单、默认值、最大 page size 与无效输入响应；查询字段不直接拼进 SQL 或 ORM 条件。
3. 演练一次兼容变更：先新增字段或版本入口，再用旧客户端请求验证旧响应仍可解析；删除/改名必须有迁移窗口和真实使用证据。
4. 只有在基线证明读取路径值得缓存时才加入缓存，并记录键、TTL、容量、写后失效责任和更新可见性。限流、熔断、指标同样需要明确保护对象、阈值、窗口、降级响应和告警接收动作。
...
- [ ] 为一个资源写出创建、读取、更新、删除、列表与筛选契约，明确每个请求的输入、成功响应、状态码、错误代码和授权条件。
- [ ] 用实际 HTTP 请求覆盖缺字段、格式错误、越界分页、未知资源、重复写入、未认证与越权；错误响应不泄露内部 SQL、堆栈或他人资源。
- [ ] 对幂等性要求明确区分：安全读取、可重试更新、创建重放和异步任务分别如何处理重复请求与请求标识。
- [ ] 写操作后验证缓存失效、列表排序和分页游标没有返回旧数据；用户私有数据不进入共享缓存键。
- [ ] 为限流、超时和依赖故障设置可观察错误与恢复路径，并在负载或故障演练中记录吞吐、错误率和延迟分布。
