...
函数参数按值传递。指针参数传递的仍是指针的值，但能通过它访问原对象。方法只是把接收者加入调用约定，不自动获得引用语义。

**本节自测**：给 struct 写值接收者与指针接收者两个加一方法，分别调用后打印原值。

<details>
<summary>预期结果与参考思路（先尝试再展开）</summary>

值接收者改的是副本；指针接收者可改原对象。别把切片或 map 字段的共享存储效果误认为 struct 没被复制。
