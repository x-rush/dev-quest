...
变量有确定类型；没有显式初值也有零值。:= 是声明，= 是给已有变量赋值，这个区别决定能否在同一作用域再次使用。

**本节自测**：声明一个未赋值的 int，打印它；再用 = 改为 3，比较第二次使用 := 的错误。

<details>
<summary>预期结果与参考思路（先尝试再展开）</summary>

输出先是 0 再是 3；同一作用域没有新变量的 := 不是赋值捷径。
...
创建 `profile.go`，声明 `name`（string）、`age`（int）、`active`（bool）和一个 `Role` 常量组：`Guest = iota`、`Member`、`Admin`。在 `main` 中打印它们，并把 `age` 转为 `float64` 后除以 2。

**验收**：`go run profile.go` 应退出成功；输出中同时有你的名字、整数年龄、`true` 或 `false`，以及 `年龄的一半: 12.5`（若年龄为 25）。把年龄改成 25.9 后再尝试直接赋给 `int` 变量：编译器应拒绝该赋值；改成 `int(25.9)` 后能运行且结果为 25。最后解释：这不是四舍五入，而是向零截断。

**提示**：先只使用 `fmt.Println`；常量组不需要变量名数组。完成后再把 `Role` 用作结构体字段，观察常量的类型如何参与赋值检查。
