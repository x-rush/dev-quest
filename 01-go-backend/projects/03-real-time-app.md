...
> **阅读准备**：理解 HTTP、goroutine、channel 与取消；先实现单进程连接管理，再考虑跨实例消息分发。

## 分阶段练习与验收

**最小阶段**：先完成两个客户端在一个房间内发收一条消息。

**验收结果**：断开一个客户端不会拖死另一个；新连接遵守明确的历史消息策略。

**扩展顺序**：先处理连接关闭、并发写与慢消费者，再扩展多实例广播。

建议保存一份正常输入、一份失败输入、实际输出和对应测试。先完成以上阶段再扩展正文中的完整设计；遇到省略实现或未定义依赖，应按文档上下文补齐，不能把代码片段拼接后当作已经验证的完整工程。

## 项目概述
本项目用聊天房间练习长连接的边界：一条消息从连接读入后，如何验证、投递、扇出、处理慢消费者并在关闭时释放资源。第一版只需要一个 Go 进程和两个本地客户端；用户体系、消息历史、跨实例广播和部署都在核心收发行为稳定后按需加入。
...
| 要解决的问题 | 先用什么 | 何时再引入扩展 | 验收重点 |
|---|---|---|---|
| 单房间连接、读写与关闭 | Go、`context`、channel 和一个 WebSocket 实现 | HTTP 路由变多时再接 Gin | 一方断开后另一方仍可收发；每条连接只有一个写入者 |
| 慢消费者 | 有界发送队列与取消策略 | 需要多房间或大量连接时再测容量 | 队列满时按明确策略断开、丢弃或限速，不能无限占内存 |
| 身份与房间权限 | 开发期受控用户 ID 或本地会话 | 需要跨服务身份时再设计 JWT/OIDC | 客户端不能仅靠修改 room/user 字段越权加入或发送 |
| 消息历史 | 内存切片，仅用于单进程演示 | 需要重启保留和查询条件时接 PostgreSQL | 明确历史条数、排序和重启清空/保留规则 |
| 跨实例广播 | 单进程 hub | 只有横向扩容时使用 Redis Pub/Sub 或其他总线 | 同一事件不会在本实例回环重复投递；断线后的恢复策略可解释 |
| 反向代理与容器 | 本地 `go test` 和最小客户端 | 需要部署时再用 Docker/Nginx | 代理升级、超时和 WebSocket header 转发由目标环境验收 |

Socket.IO 不是 Gorilla WebSocket 的必备搭配；只有客户端协议确实要求 Socket.IO 时才选择其服务端实现。本文的 Redis、PostgreSQL、JWT、Docker 和 Nginx 片段是后续集成设计，不构成本页已运行的外部服务验证。
