...
先让标准库 `flag`、`os/exec`、`context` 和 `testing` 解决一个子命令的输入、执行、取消和测试。下面的库是本页后续工程片段实际采用或可选比较的工具；它们不是必须一次性安装的“现代 CLI 套餐”。

| 能力 | 本页采用或比较的工具 | 在哪一阶段引入 | 不引入时的做法与代价 |
|---|---|---|---|
| 子命令、帮助与 shell 补全 | Cobra | 阶段 1 的命令变多，或阶段 7 要补全时 | `flag` 适合单层命令；自己维护子命令帮助、补全和参数分派会逐渐变复杂 |
| 多来源配置 | Viper | 阶段 2 需要合并文件、环境变量和 flag 时 | 直接解析 YAML/JSON 并显式合并更容易看清优先级；Viper 不能替代业务字段校验 |
| 执行外部任务与取消 | 标准库 `os/exec`、`context` | 阶段 4 | 不要用 shell 字符串拼接代替参数数组；`CommandContext` 才能把父级取消传给子进程 |
| 并发上限与依赖调度 | 标准库 `sync`、channel | 阶段 5 | 顺序执行更易诊断；并发只在任务彼此独立且有测量依据时引入 |
| 结构化日志 | Zap（正文代码使用）或标准库 `log/slog` | 阶段 6 | `fmt` 足够做交互提示，却难以按 task、duration、error 检索；无论选哪个都不得记录密钥 |
| 进度显示 | `progressbar/v3` | 阶段 7，可选 | CI、重定向输出和短任务不应强制动态进度条；先确保 stdout/stderr 与退出码稳定 |
| 断言辅助 | 标准库 `testing` 为基础，Testify 为可选断言库 | 每个阶段 | Testify 减少重复断言文字，但不替代边界输入、失败路径和可观察行为的设计 |
| 方案比较 | `urfave/cli` | 选型练习，不与 Cobra 混装 | 选择其中一个命令框架；同一二进制同时维护两套路由会让帮助、flag 和测试语义分裂 |
