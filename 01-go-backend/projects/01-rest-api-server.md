...
这是一个工程化扩展练习，不是一份“所有 Go API 都要使用这些库”的清单。先完成本模块的标准库 TODO CLI，再只为本项目新增下列能力；每增加一个依赖都要说明替代方案和验证方式。

| 职责 | 本项目的候选实现 | 为什么需要它 | 最低验证 |
|---|---|---|---|
| HTTP 路由和中间件 | Gin 或 `net/http` | 将 HTTP 请求映射为已有的领域操作 | 非法 JSON、缺少字段、未知资源分别返回约定状态码；不把领域错误原文泄露给客户端 |
| 持久化 | PostgreSQL；先用 `database/sql`，需要映射辅助时再评估 GORM | 重启后仍保存数据，并以事务保护相关写入 | 重复请求、事务失败和重启后读取的结果可复现 |
| 身份与授权 | 项目定义的会话或 token 方案 | 区分“谁在调用”和“能否操作这条资源” | 伪造／过期凭据为 401，越权访问为 403，服务端不能只靠前端隐藏按钮 |
| 配置与密钥 | 环境变量和显式配置结构；Viper 仅在确有多来源合并需求时使用 | 将部署差异放在代码外，密钥不进入仓库 | 缺少必要配置时启动失败但不打印密钥；测试可注入独立配置 |
| 日志 | 标准库 `log/slog` 或团队既有日志适配器 | 以 request ID、操作、状态和耗时定位问题 | 日志可关联一次失败请求，且不包含令牌、密码或完整敏感正文 |
| 测试 | `go test`、`httptest` 与受控数据库夹具 | 验证 HTTP 契约与持久化边界 | 成功、输入失败、授权失败、存储失败和重试至少各有一例 |
...
4. **项目架构**
   - handler 只解析请求、取得身份并写 HTTP 响应；service 执行业务规则；repository 只读写存储
   - 启动入口通过构造函数提供数据库实现；测试用内存或替身 repository 验证同一条业务规则
   - 验收：替身返回“用户不存在”时接口为约定的 404；替身返回存储故障时为安全的 5xx，且没有成功写入
...
**项目价值**: 这个项目把请求校验、认证、业务规则、持久化和错误响应连成一条可测试路径。完成时应保留启动命令、迁移版本、成功与失败请求样本和测试结果；目录齐全或容器能启动不等于接口行为已经验收。
