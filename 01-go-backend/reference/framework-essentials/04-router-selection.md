...
| **标准库兼容性** | 中：`gin.Context` 包裹了 Request/Writer，与原生中间件混用需适配 | **完全兼容**：任何 `http.Handler`/`http.Middleware` 直接可用 | 中：自有 Context，需经适配器使用原生组件 |
...
| **迁移自由度** | 较低（业务代码与 gin.Context 耦合） | **最高**（路由器可随时替换，业务代码不动） | 较低（echo.Context 耦合） |
...
- ✅ **正确做法**：路由层差异在真实业务中占比极小，迁移成本与生态匹配才是决定因素。
