...
下面的完整示例把声明、方法、接口和函数值放进同一程序；请使用本页列出的命令运行它，并将输出与预期结果对照：
...
接口也可限定类型参数，如 `interface{ ~int | ~int64 }`；`~` 表示底层类型。这种包含类型项的非基本接口只能用作约束，不能声明普通接口变量。[泛型参考](09-generics.md)进一步解释类型集合。Go 的方法不能声明自己的类型参数；需要额外类型参数时，改用普通泛型函数，或把类型参数放在接收者所属的泛型类型上。接口方法同样不能声明类型参数。[官方语言变更](https://go.dev/doc/go1.27#language)应与项目实际 Go 版本一起核对。
