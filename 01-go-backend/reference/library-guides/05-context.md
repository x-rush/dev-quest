...
context 是 Go 传递**请求作用域的截止时间、取消信号与请求级数据**的标准机制。心智模型——**一棵从请求入口生长的树**：父节点取消，所有子孙节点同时收到信号；每个阻塞操作（DB 查询、HTTP 调用、channel 等待）都应感知 ctx 以便"随时可中断"。
...
**取消传播机制**：`Done()` 返回的 channel 被关闭是广播——所有 `select` 等待者同时唤醒。`cancel` 是幂等的，多调无害；不调用则 ctx 及其子树在父取消前一直占内存（轻微泄漏），规范写法 `defer cancel()`。
...
func main() {
	...
	defer rootCancel()
	childA, _ := context.WithCancel(root)
	childB, _ := context.WithCancel(root)
	go func() {
		time.Sleep(10 * time.Millisecond)
		rootCancel()
	}()
	<-childA.Done()
	<-childB.Done() // 父取消 → 子树全部完成
...
- ✅ **正确做法**：检查 `ctx.Err()`；确需"脱离取消"（如审计日志必须写完）用 `context.WithoutCancel` 显式构造。
