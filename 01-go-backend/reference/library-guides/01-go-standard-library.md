...
前置：import、函数返回值和 error。标准库无需第三方下载，但文件、网络和数据库连接仍可能失败。学包时按“创建什么 → 谁使用 → 谁关闭 → 错误交给谁”读，比按 API 字母顺序背诵更有效。

| 任务 | 首先查什么 | 必须确认的边界 |
|---|---|---|
| 文本转数值 | strconv | 非法文本与溢出都会返回错误，不要忽略 err |
| JSON 数据交换 | encoding/json | 字段导出、标签、缺失字段与数字表示 |
| 读小文件 | os.ReadFile | 一次读入内存；大文件改为流式处理 |
| 按行或分块处理 | io / bufio | EOF 与真实读取错误不同，Scanner 还有缓冲限制 |
| 请求服务 | net/http / context | 超时、取消、状态码与响应体关闭 |
| 数据库操作 | database/sql | 连接池、事务、Rows 关闭和 Scan 错误 |
