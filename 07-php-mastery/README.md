...
版本号会变化，课程只固定语言能力与验证流程。PHP 8.5 用于讲解管道运算符、URI 扩展、`#[\NoDiscard]` 和 `clone()` 更新属性；部署或旧项目可以选择仍受支持的 8.3/8.4，但必须避开当前环境没有的语法。创建、升级或排错时先看 [PHP 支持周期](https://www.php.net/supported-versions.php)、[PHP 下载页](https://www.php.net/downloads.php) 和框架自己的系统要求。

| 项目 | 教学默认 | 创建或升级时要验证什么 |
|------|------|------|
| **PHP** | **8.5**；示例明确标注 8.4/8.5 专属语法 | `php -v`、`php -m` 与项目 CI 一致；确认目标扩展已启用，再运行 `composer check-platform-reqs`。不要把本机能运行等同于服务器有相同扩展。 |
| **Laravel** | 当前项目使用的稳定主线，由 `composer.lock` 固定 | 通过 `composer create-project` 或现有锁文件生成依赖；升级主版本前阅读迁移指南、运行测试并检查 PHP 版本与扩展约束。框架版本不能替代 API、数据库和队列的集成测试。 |
| **Composer** | **2.x**，每个项目独立锁定 | 提交 `composer.json` 与 `composer.lock`；在 CI 使用 `composer install --no-interaction --prefer-dist`，而不是在构建阶段无约束地 `update`。 |
| **PHPUnit / Pest** | 按项目 PHP 版本选择 | PHPUnit 13 要求 PHP 8.4+；PHP 8.3 项目应使用兼容的 PHPUnit 12。先清除当前版本的弃用告警再计划跨主版本升级，执行 `vendor/bin/phpunit` 或项目的 Pest 命令记录结果。 |

这张表不宣称所有项目已安装上述依赖；真实可用版本由锁文件、`composer check-platform-reqs` 和实际测试共同证明。
