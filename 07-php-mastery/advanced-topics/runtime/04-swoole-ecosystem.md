...
- **用户态切换**：协程切换由运行时调度，通常比创建进程或线程轻；可同时存活多少协程取决于每个协程的栈、请求对象、连接数和内存上限，必须在目标负载下测量，不能把“万级”当作容量承诺
- **PHP 语法层的对应物是 Fiber**（8.1+）：Fiber 定义“可挂起/恢复”的语言原语；Swoole 协程是 C 扩展层的独立实现。两者在“挂起与恢复”的概念上可对照，但调度、I/O hook、栈管理和生态集成不同，不能只凭实现层判断性能；应以目标 PHP/扩展版本和实际 I/O 负载比较。
- **调度发生在 I/O 边界**：CPU 计算不会自动让出——协程世界里"长时间循环计算"同样会卡住整个事件循环，纪律与 Workerman 一致

### 概念二：一键协程化（hook）——同步写法，异步执行

**定义**: Swoole 的 hook 机制可把一部分受支持的阻塞 API 改为协程感知的调用：代码仍保持同步写法，但在受支持的 I/O 等待点让出执行权。支持范围、启用方式和边界随扩展版本、hook flags 与客户端库而变；未被 hook 的调用仍会阻塞当前事件循环。
...
Co\run(function (): void {
    ...
    // 两个同步风格的请求；只有目标扩展、hook flags 和客户端均支持时，等待才会重叠。
    go(function (): void {
    ...
    // 观察总耗时是否接近较慢请求，而不是两段等待之和；网络、DNS、连接建立和失败重试都会改变结果。
    co::sleep(1.2);
...
- **同步风格可复用**：受支持 API 通常不用改成回调；先用一个并发 I/O 例子和超时/错误用例确认实际库已被 hook，再决定是否迁移旧代码
...
协程让等待 I/O 的任务有机会让出执行权，但不会把纯计算自动分到多个 CPU。把慢计算移到合适的进程或执行器，并限制并发量；给每个请求无限创建协程可能转而压垮数据库。

连接池应匹配协议与客户端契约，借出的连接要正确归还且不能跨请求残留事务。hook 只覆盖支持的操作；在目标扩展版本中检查是否仍会阻塞。连续交错运行两个租户请求，检查上下文、连接与错误路径是否隔离，再观察持续负载下的内存。
...
| 并发原语 | 单进程内协程；容量由内存、连接数、限流与 I/O 模型共同决定 | 多进程 + 事件循环；容量还受每进程连接与内存上限约束 |
| 阻塞库兼容 | 对受支持 API 可启用 hook；不支持的调用仍需异步客户端、进程隔离或改造 | 事件循环中避免阻塞；需要异步客户端或把阻塞工作移出循环 |
