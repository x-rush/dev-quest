...
- ✅ **聊天列表一律 `inverted`**，别手写"自动滚到底"的 offset 计算
- ✅ **乐观更新 + 失败补偿**，发送体验优先于发送确认
- ✅ **重连用指数退避**，固定间隔重连会在服务端故障时形成流量风暴
- ✅ **`data` 用函数式 setState 更新**，避免闭包旧值丢消息
- ❌ **不要高频全量替换大数组引用**，拷贝成本在高频下被放大；可考虑游标分页
- ❌ **不要在 `renderItem` 里创建新函数/新对象**传给子组件，会击穿 memo
