...
**最小阶段**：从前一个项目挑一条主要用户流程，补测试和构建。

**验收结果**：干净设备可安装，核心流程可走通，出现失败能取得诊断信息。

**扩展顺序**：签名、商店提交、升级与回退分别记录证据，不以打出包为终点。
...
发布标准应来自应用自己的设备基线与用户目标。例如测冷启动、关键操作成功率和崩溃影响人数，明确统计时间窗和样本量，再定义停止放量的条件；固定的 0.5% 或 2 秒不能代表所有应用合格。

先向受控群体发布并验证升级、登录和数据兼容，保留暂停与修复路径。客户端公开变量不放服务秘密，凭据的有效期、撤销和刷新按身份系统设计。测试随功能进入流水线，更新渠道遵守平台规则及原生兼容边界。
