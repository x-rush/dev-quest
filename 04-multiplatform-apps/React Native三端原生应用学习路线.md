...
> **阅读准备**：会 JavaScript 基本编程；尚不熟 React 时先学习组件、props、state 和 effect，再进入原生构建与平台适配。
...
先选择一台可获得的目标设备，完成安装、网络请求、本地保存和权限拒绝四条路径，再扩展另一平台。这样能把“React 写法不熟”和“平台能力不同”分开定位，学习顺序由设备与产品需求决定。

复用多少代码没有固定百分比。列表与数据处理通常容易共享，系统权限、后台任务和特定硬件往往需要平台实现；用一个明确接口隔离差异，再分别测试。商业化练习可以选一个真实小需求，观察用户能否完成任务，而不是把生态热度当作产品价值的证据。
