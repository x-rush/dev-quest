...
E2E 验证安装后的应用能否完成核心旅程，例如创建待办、重启并再次看到它；分支规则尽量用更快的测试覆盖。测试账号和数据要隔离，第三方波动不能混进每一次基础回归。

失败时保存截图、设备日志和构建版本，先确定卡在哪一步。视觉回归可以是 E2E 的一部分，但需要稳定字体、尺寸和渲染环境；不是一概禁止截图比较。对异步界面等待状态变化，避免用长 sleep 掩盖未完成的操作。
...
<!-- acceptance-exercise -->
## 练习与验收：在真实运行入口完成一个数据闭环

安装测试包，用专用账号创建带唯一标识的任务，退出详情再进入，确认内容存在；重启应用后再检查约定的持久化结果。接着断网创建，预期出现产品约定的失败或待同步状态，不显示虚假成功。验收保存失败截图、设备日志与应用版本，清理仅针对本轮创建的数据；重跑不能依赖上次遗留任务。
