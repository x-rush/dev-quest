...
**本节自测**：先完成正文提供的记账页，录入 `0.10` 和 `0.20`，关闭并重新打开应用，确认仍显示两条记录且合计为 0.30 元；待办新增与完成作为第二阶段。
...
**交付分三阶段**：入门必做仅要求一个已在[环境搭建](./01-environment-setup.md)中运行成功的 Android 或 iOS 目标。先会使用 `useState`、异步函数和受控输入；不熟时返回[状态与 Hooks](./04-state-hooks.md)。

| 阶段 | 交付产物 | 通过条件 |
|---|---|---|
| 1：单平台记账 | 学习者自己的 Expo 工程、依赖锁文件、记账 Hook 和页面 | `0.10`、`0.20` 合计 0.30；非法金额被拒绝；重启读回；写入失败保留表单且列表不变 |
| 2：完整业务 | 三 Tab 应用；统计放入“我的”页，待办按需求自行实现 | 新增 A/B、完成并删除 B 后重启，A 在、B 不在；切换 Tab 不丢成功保存的数据 |
| 3：平台扩展（选做） | 第二平台或独立 RNOH 工程及各自验收记录 | 在对应设备重新执行前两阶段；未运行的平台标为未验证 |

每阶段保存设备/系统、启动方式、依赖版本和实际结果。热更新留到[OTA 部署](../deployment/03-ota-updates-observability.md)，不作为首项目通过条件。第一阶段完成后，按[待办应用](../projects/01-todo-app.md)补齐待办业务，再返回第二阶段。
...
**任务要求**:
1. 在一个已选定的平台完成上述第一阶段，再实现第二阶段的全部 5 项功能
2. 在这个平台的真机或模拟器执行交互验收；EAS Build 成功只证明构建，不替代运行验收
3. 第二平台与鸿蒙归入选做的第三阶段，分别记录构建和交互结果
